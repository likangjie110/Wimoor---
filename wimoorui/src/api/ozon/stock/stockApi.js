...
function getTaskDetail(authId, taskId) {
  return request.get(`/ozon/api/v1/stock/task/${taskId}/detail`, {
    params: { authId }
  });
}
...
function listTaskHistory(authId, limit) {
  return request.get("/ozon/api/v1/stock/task/history", {
    params: { authId, limit }
  });
}
...
function listTasksBySku(authId, sku) {
  return request.get("/ozon/api/v1/stock/task/by-sku", {
    params: { authId, sku }
  });
}
...
function getErrorSummary(authId) {
  return request.get("/ozon/api/v1/stock/task/error-summary", {
    params: { authId }
  });
}
