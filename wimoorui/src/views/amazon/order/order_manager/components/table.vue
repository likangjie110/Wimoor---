<template>
	<Header @refreshTable="getdataArea"   @downloadOrderlist="downloadOrder"   @callbackReview="callbackReview"   />
	<el-row>
	   <GlobalTable ref="globalTable" :tableData="tableData" @selectionChange="selectionChange"  height="calc(100vh - 300px)" @loadTable="loadtableData" :defaultSort="{ prop: 'buydate', order: 'descending' }"  :stripe="true"  style="margin-bottom:16px;">
		   <template #field>
		   ...
		   <el-table-column prop="remark"   label="备注" width='100' >
	  	    ...
	  	    <el-table-column prop="operate" label="操作"  width="180"   sortable="custom">
	  	        <template #default="scope">
	  	          <el-space>
...
<script setup>
	...
	function getdataArea(rows){
		 getDataParams(rows)
	}
	function showMessageModal(row){
		messageRef.value.show(row);
	}
	function downloadOrder(rows){
		 downloadOrderXlsx(rows)
	}
