	  ...
	  ref="globalTable" :tableData="tableData"   height="calc(100vh - 260px)" @loadTable="loadTableData" :stripe="true"  style="width: 100%;margin-bottom:16px;">
		 ...
		 <el-table-column prop="totalprice"    sortable="custom"    label="物流总费用数量" width="140" >
		 			 <template #default="scope">
		 				  ...
		 				  <div><span class="font-extraSmall">运输重量:</span>{{scope.row.transweight}}{{scope.row.wunit}}</div>
						  <div><span class="font-extraSmall">运输单价:</span>{{scope.row.singleprice}}</div>
		 ...
		 <el-table-column prop="ShipmentId"   sortable="custom"   label="货件编码" width="150" >
			  ...
			  <el-table-column prop="groupname"   sortable="custom"    label="发货店铺" width="100" ></el-table-column>
			   <el-table-column prop="market"     sortable="custom"  label="收货站点" width="100" >
				...
				<el-table-column prop="shiped_date"    sortable="custom"   label="发货日期" width="180" >
						   <template #default="scope">
									  <div> {{dateFormat(scope.row.shiped_date)}}</div>
									   <div><span class="font-extraSmall">到货日期:{{dateFormat(scope.row.arrivalTime)}}</span></div>
						   </template>
				</el-table-column>
				<el-table-column prop="market"    sortable="custom"   label="发货渠道"  >
						   <template #default="scope">
									  <div> {{scope.row.channame}}</div>
									  <div><span class="font-extraSmall">预计到货日期:{{scope.row.deliveryTime}}
									  </span></div>
						   </template>
				</el-table-column>
				<el-table-column prop="quantity"     sortable="custom"  label="发货数量" width="100" ></el-table-column>
				<el-table-column prop="received"     sortable="custom"  label="到货数量" width="100" ></el-table-column>
				<el-table-column prop="shipmentstatus"     sortable="custom"  label="状态" width="130" ></el-table-column>
				<el-table-column prop="createtime"     sortable="custom"  label="创建日期" width="100" >
					<template #default="scope">
