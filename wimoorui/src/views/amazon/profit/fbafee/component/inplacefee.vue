			 ...
			 <div class="con-header" >
			 	  <el-row>
					<el-space >
					  ...
					  <el-select v-model="queryParam.dispatchtype"  filterable   @change="handleQuery">
						<el-option label="全部" key="" value=""></el-option>
					  	<el-option v-for="item in inplaceList" :label="item.name"
					  	 :key="item.id" 
					  	 :value="item.id">
					  	</el-option>
					  </el-select>
			   ...
			   <template #field>
			 	 <el-table-column prop="id" label="ID"  sortable='custom' width="80" >
			 		 <template #default="scope">
			 			 <el-input v-if="scope.row.edit" v-model="scope.row.id" type="number" ></el-input>
			 			 <div v-else>{{scope.row.id}}</div>
			 		  </template>
			 	  </el-table-column>
			 	  <el-table-column prop="inplacefeeid" label="目标ID"  sortable='custom'  width="220" >
			 	  	<template #default="scope">
			 	  		<el-select v-model="scope.row.inplacefeeid" :disabled="!scope.row.edit" filterable  >
			 	  			<el-option v-for="item in inplaceList" :label="item.name"
			 	  			 :key="item.id" 
			 	  			 :value="item.id">
							</el-option>
			 	 ...
			 	 <el-table-column prop="producttierId" label="尺寸分段" sortable='custom'   width="300" >
			 	 	<template #default="scope">
				 ...
				 <el-table-column prop="standard" label="standard"  width="80"  >
				 	<template #default="scope">
				 			<el-checkbox v-model="scope.row.standard" :disabled="!scope.row.edit" size="large" />
				 	 </template>
				 </el-table-column>	
				 ...
				 <el-table-column prop="description" label="description" width="280" show-overflow-tooltip=""  >
				 	<template #default="scope">
...
<script setup>
	...
	function loadTableData(param){
		profitParamApi.getInplaceFeeFormat(param).then((res)=>{
	...
	function saveItem(row){
		if(row.id){
			profitParamApi.updateInplaceFeeFormat(row).then((res)=>{
				 ElMessage.success('操作成功！');
				 handleQuery();
			});
		}else{
			profitParamApi.saveInplaceFeeFormat(row).then((res)=>{
				 ElMessage.success('操作成功！');
				 handleQuery();
			});
		}
	...
	function deleteItem(row){
		ElMessageBox.confirm(
			    ...
			    '删除FBA费用',
			    {
				  ...
				  profitParamApi.deleteInplaceFeeFormat(row).then((res)=>{
	 ...
	 onMounted(()=>{
		...
		profitParamApi.getInplaceList(state.queryParam).then(res=>{
				state.inplaceList=res.data;	 
