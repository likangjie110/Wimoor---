 ...
 <div class="summary-top expend-table" v-loading="selectionloding">
	 <GlobalTable  ref="globalTable" :tableData="tableData"
	   ...
	   >
		 ...
		 <el-table-column fixed    label="广告信息"  width="200" show-overflow-tooltip>
			   <template #default="scope">
					<div v-if="scope.row.name" class='name'>广告名:{{scope.row.name}}</div>
					<div   class='name'>Asin:{{sbAdsFormatter(scope.row)}}</div>
			   </template>
		  </el-table-column>
...
<script setup>
	...
	import {adgroupsuggestFormatter,sbAdsFormatter} from '../javascript/formatter.js';
