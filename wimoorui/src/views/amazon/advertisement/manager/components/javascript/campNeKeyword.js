...
function handleApiData(state,res){
		...
		if(state.tableData.records && state.tableData.records.length>0){
			...
			//加载各个ext信息
			advKeywordsApi.campKeywordNegativaExt(profileid+"",state.queryParams.campaignType,data).then((res)=>{
...
async function getData(profileid,campaignType,data){
		var campaigns=[];
		await advKeywordsApi.campKeywordNegativa(profileid,campaignType,data).then((res)=>{
			campaigns=res.data;
		 });
		 return campaigns;
	}
 ...
 export function update(state,arrays,callback){
 	...
 	advKeywordsApi.updateCampNegativaKeywordList(state.queryParams.profileid,state.queryParams.campaignType,{"jsonstr":addstr}).then((res)=>{
