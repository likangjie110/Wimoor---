...
function handleApiData(state,res){
		...
		if(state.tableData.records && state.tableData.records.length>0){
			...
			//加载各个ext信息
			advTargetApi.campNegativaTargetsExt(profileid,state.queryParams.campaignType,data).then((ress)=>{
...
export function loadList(state){
 ...
 async function getData(profileid,campaignType,data){
 		var campaigns=[];
 		await advTargetApi.campNegativaTargets(profileid,campaignType,data).then((res)=>{
 			campaigns=res.data;
 		 });
 		 return campaigns;
 	}
 ...
 export function update(state,arrays,callback){
 	...
 	advTargetApi.updateCampNegativaTargets(state.queryParams.profileid,state.queryParams.campaignType,{"jsonstr":addstr}).then((res)=>{
