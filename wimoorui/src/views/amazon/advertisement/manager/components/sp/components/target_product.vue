					...
					<el-tab-pane label="商品" name="asin">
						<el-tabs v-model="activeProduct" @tab-change="handleAsinChange">
							<el-tab-pane label="建议商品" name="suggestAsin">
								<el-space>
									<el-checkbox-group size="small" v-model="asinTargetType" @change="changeTargetType">
								...
								<el-table :data="suggestProductList" height="368px" v-loading="suggestasinLoading">
									...
									<el-table-column label="ASIN" prop="recommendedAsin"  show-overflow-tooltip>
										<template #default="scope">
											<div>{{scope.row.recommendedAsin}}</div>
							...
							<el-tab-pane label="搜索" name="searchAsin">
								...
								<el-checkbox-group v-model="asinTargetType" @change="changeTargetType" size="small">
									<el-checkbox label="ASIN_SAME_AS">精准</el-checkbox>
									<el-checkbox label="ASIN_EXPANDED_FROM">扩展</el-checkbox>
								</el-checkbox-group>
								<BiddingType  @change="handleListBid" :bidType="bidType" ></BiddingType>
...
<script setup>
    ...
    function loadData(){
		if(state.queryParams.querylist&&state.queryParams.querylist.length>0){
			state.activeTab="asin";
			state.activeProduct="writeAsin";
			var targetlist=[];
			state.suggestCategoryList=[];
			...
			if(targetlist && targetlist.length>0){
				targetlist.forEach(item=>{
					var row={};
					row.iamge=null;
					row.asin=item.keywords.toUpperCase();
					row.expression={"type":"ASIN_SAME_AS","value":row.asin};
	...
	function show(params) {
		...
		state.targetingType=params.campaign.targetingType.toUpperCase();
		loadData();
	}
	defineExpose({
		show,
	...
	function addRefine(){
		...
		state.refineForm.brand.forEach(brandid=>{
			...
			var expression=[];
			expression.push({"type":"ASIN_CATEGORY_SAME_AS","value":state.refineForm.row.id+""});
	...
	function  loadCateById(){
		...
		advTargetApi.getTargetsCategoriesAllRecommendations(state.queryParams.profileid,state.queryParams.campaignType,param).then((res)=>{
			...
			if(res.data){
				...
				state.classtree=JSON.parse(data.CategoryTree);
			}
		});
	}  
	...
	function addCateTarget(row){
		var expression=[];
		expression.push({"type":"ASIN_CATEGORY_SAME_AS","value":row.id});
	...
	function addAsinTarget(row){
		 state.asinTargetType.forEach(item=>{
			...
			var rows=JSON.parse(JSON.stringify(row));
			 ...
			 if(rows.recommendedAsin){
				rows.asin=rows.recommendedAsin; 
			 }
			 expression.push({"type":item,"value":rows.asin});
	...
	function loadAllCategoryProductCount(){
		state.suggestCategoryList.forEach(item=>{
			...
			loadCategoryProductCount(param,datas=>{
				if(datas){
					item.AsinCountsMin=datas.AsinCounts[0].min;
					item.AsinCountsMax=datas.AsinCounts[0].max;
				}
			})
		});
	}
	...
	function loadSuggestTargetBid(){
		state.suggestCategoryList.forEach(item=>{
			...
			var targets=[];
			var list=[];
			var param={"type":"asinCategorySameAs","value":parseInt(item.id)   };
			list.push(param);
			targets.push(list);
			advTargetApi.getTargetBidRecommendations({"profileid":state.queryParams.profileid,"expression":targets,
			"adtype":state.queryParams.campaignType,"adGroupid":state.queryParams.adgroupid,
	...
	function loadAsinSuggestBid(){
		state.suggestProductList.forEach(item=>{
			...
			var targets=[];
			var list=[];
			var param={"type":"asinSameAs","value": item.recommendedAsin   };
			list.push(param);
			targets.push(list);
			advTargetApi.getTargetBidRecommendations({"profileid":state.queryParams.profileid,"expression":targets,
			"adtype":state.queryParams.campaignType,"adGroupid":state.queryParams.adgroupid,
	...
	function loadAsinInfo(asinlist){
		 ...
		 asinlist.forEach(item=>{
		 	if(item.asin){
		 		...
		 		list.push(item.recommendedAsin);
		 	}
	...
	function loadSuggestTargetAsin(){
		...
		advTargetApi.getTargetsProductsRecommendations(state.queryParams.profileid,state.queryParams.campaignType,params).then((res)=>{
			...
			var datas=JSON.parse(res.data);
			state.suggestProductList= datas.recommendations;
	...
	function loadSuggestTargetCategories(){
		...
		var param={"includeAncestor":true, "asins":asins};
		advTargetApi.getTargetsCategoriesRecommendations(state.queryParams.profileid,state.queryParams.campaignType,param).then((res)=>{
			...
			if(res.data){
					var data=JSON.parse(res.data);
					state.suggestCategoryList=data.categories;
