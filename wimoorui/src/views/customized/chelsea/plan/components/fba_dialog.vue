<template>
		<el-dialog v-model="visible" title="创建发货单" width="80%" top="3vh" center >
		<el-row>
			<el-col :span="20">
...
let editRef=ref();
let prodiaRef=ref();
const emit = defineEmits(['change','cancel','save']);
let state=reactive({
...
function loadInplace(){
 ...
 watch(() =>router.currentRoute.value.query,(newValue,oldValue)=> {
		...
		function show(orderData){
			 state.visible=true;
			 state.formData.groupid=orderData.groupid;
			 state.formData.marketplaceid=orderData.marketplaceid;
			 state.formData.invtype=2;
			 if(orderData.entryList && orderData.entryList.length>0){
				 orderData.entryList.forEach(function(item){
					 item.id=item.materialid;
				 });
				state.productlist=orderData.entryList;
				state.totalproducts=state.productlist.length;
