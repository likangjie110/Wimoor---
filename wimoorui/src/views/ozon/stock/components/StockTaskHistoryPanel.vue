  ...
  >
    <div class="task-history-panel">
      ...
      >
        ...
        <el-table-column label="仓库ID" width="140" show-overflow-tooltip>
    ...
    >
      <div v-if="currentTask" class="task-detail">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="仓库ID">{{ currentTask.warehouseId || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="操作员" :span="2">{{ currentTask.operator || '-' }}</el-descriptions-item>
        </el-descriptions>
        ...
        <div v-if="currentTask.itemResults && currentTask.itemResults.length > 0" class="item-results">
          ...
          <el-table :data="currentTask.itemResults" border size="small" max-height="400">
            ...
            <el-table-column label="请求库存" width="100" align="center">
              <template #default="scope">
                {{ scope.row.requestedStock || 0 }}
              </template>
            </el-table-column>
            <el-table-column label="实际库存" width="100" align="center">
              <template #default="scope">
                {{ scope.row.actualStock || 0 }}
              </template>
            </el-table-column>
            <el-table-column label="状态" width="100" align="center">
              <template #default="scope">
...
async function loadTaskHistory() {
  ...
  loading.value = true;
  try {
    const res = await stockApi.listTaskHistory(props.authId, props.limit);
    if (res.code === 200) {
...
async function loadErrorSummary() {
  ...
  try {
    const res = await stockApi.getErrorSummary(props.authId);
    if (res.code === 200) {
...
async function openDetail(task) {
  try {
    const res = await stockApi.getTaskDetail(props.authId, task.taskId);
    if (res.code === 200) {
