  ...
  <div class="component-container">
    ...
    >
      ...
      <el-table-column label="菜单类型" align="center" width="100">
        <template #default="scope">
			<el-tag v-if="!scope.row.path&&!scope.row.redirect" type="warning"
			  >目录</el-tag
			>
          <el-tag v-else-if="scope.row.path&&scope.row.path!=''" type="success">菜单</el-tag>
   ...
   setup(props,context){
	   ...
	   async function handleUpdate(row) {
	     ...
	     getMenuDetail(id).then(({ data }) => {
	       state.formData = data;
		   if(data.redirect){
			 cacheData.value.menuType =  "EXTLINK";
		   }else if(data.path){
			 cacheData.value.menuType =  "MENU";
		   }else{
			   cacheData.value.menuType =  "CATALOG";
		   }
	      data.type=cacheData.value.menuType;
	    ...
	    * 菜单类型 change
	    */
	   function handleMenuTypeChange(menuType) {
	   ...
	   return {selected,cancel,handleDelete,submitForm,
	           handleUpdate,handleAdd,loadMenuData,handleQuery,
			   resetQuery,handleMenuTypeChange,handleRowClick,
			   //function
		      loading,state,
		      queryParams,menuList,
		      dialog,formData,
		      rules,menuOptions,
		      iconSelectVisible, cacheData,//modal
			  queryFormRef,dataFormRef,popoverRef,
			  //ref
	   }
   }
 };
