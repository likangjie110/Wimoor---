...
export default {
  ...
  setup(props,context){
		...
		function loadDictOptions() {
		  getDictItemsByTypeCode('micro_service').then((response) => {
			state.microServiceOptions = response.data;
		  });

		 getDictItemsByTypeCode('request_method').then((response) => {
		...
		function submitForm() {
		  dataFormRef.value.validate((isValid) => {
			if (isValid) {
			  ...
			  formData.value.urlPerm=state.urlPerm.requestMethod +
				                     ':/' +
				                     state.urlPerm.serviceName +
				                     state.urlPerm.requestPath;
	  ...
	  return{
		  proxy,queryFormRef,dataFormRef ,//ref
		  state,
		  loading,multiple,
		  permList, total,
		  dialog,formData,
		  rules,microServiceOptions,
		  requestMethodOptions,urlPerm,
		  queryParams,//model
		  handleQuery,cancel,resetForm,submitForm,
		  handleUpdate,handleAdd,loadDictOptions,handleSelectionChange,
		  resetQuery,loadDictOptions,handleDelete
		  //function
	  }
  }
};
