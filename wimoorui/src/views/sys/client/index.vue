...
function handleSelectionChange(selection) {
  state.ids = selection.map((item) => item.clientId);
  state.single = selection.length !== 1;
  state.multiple = !selection.length;
}

function handleAdd() {
  proxy.$getDictItemsByTypeCode('grant_type').then((response) => {
...
function handleUpdate(row) {
  ...
  proxy.$getDictItemsByTypeCode('grant_type').then((res) => {
