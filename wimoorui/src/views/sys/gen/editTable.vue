      ...
      <el-tab-pane label="字段信息" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          ...
          <el-table-column label="Java类型" min-width="11%">
            <template #default="scope">
              <el-select v-model="scope.row.javaType" style="width:120px">
          ...
          <el-table-column label="查询方式" min-width="10%">
            <template #default="scope">
              <el-select v-model="scope.row.queryType" style="width:100px">
          ...
          <el-table-column label="字典类型" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.dictType" style="width:120px" clearable filterable placeholder="请选择">
                <el-option
...
import { getGenTable, updateGenTable } from "@/api/sys/tool/gen.js"
import { listPageDictTypes } from "@/api/sys/admin/dict.js";
...
function close() {
  ...
  return router.push({ path: "/sys/gen",
    query: { t: Date.now(),
...
(() => {
  ...
  if (tableId) {
    ...
    /** 查询字典下拉列表 */
    listPageDictTypes({"limit":10000,"page":1}).then(response => {
