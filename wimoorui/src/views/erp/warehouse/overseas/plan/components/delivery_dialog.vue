<template>
	<el-dialog top="5vh" class="transdialog" v-model="dialog.visible" title="发货配置" @open="handleOpen" width='1100px'>
...
<script setup>
	...
	import cycleApi from '@/api/erp/order/cycleApi.js'
   ...
   function handleQuery(){
	   cycleApi.list(state.queryParams).then(res=>{
		  var countrydata={};
		   if(res&&res.data){
		   ...
		   state.countryOptions.forEach(item=>{
			   ...
			   if(countrydata[item.market]){
				   var image=$require('country/'+item.market+'-flag-small.jpg') ;
				   countrydata[item.market].image=image;
				   countrydata[item.market].name=item.name;
				   state.formData.push(countrydata[item.market]);
			   }else{
				   var isdefault=false;
   ...
   function submitForm(){
	   ...
	   cycleApi.save(state.formData).then(res=>{
