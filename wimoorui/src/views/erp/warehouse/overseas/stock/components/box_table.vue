	   ...
	   border="0" cellpadding="0" cellspacing="0"  style="min-height: 600px;" >
		<thead  >
			<tr  >
				...
				<th width="80px;" > 
					<el-tag   type="default" @click="inputboxNum=inputboxNum+1;  refreshsize();" link>+</el-tag>
					<el-tag  style="margin-left:5px;"  type="primary" @click="inputboxNum>1?inputboxNum=inputboxNum-1:1;  refreshsize();" link>-</el-tag> 
				</th>
			</tr>
		</thead>
		<tbody>
...
<script setup>
	...
	function submitBox(type){
		...
		dispatchApi.savePackingInformation(params).then((res)=>{
			if(opttype=='save'){
...
function loadBoxData(){
	...
	function getBoxDetial(){
		...
		dispatchApi.getBoxDetail(params).then((res)=>{
