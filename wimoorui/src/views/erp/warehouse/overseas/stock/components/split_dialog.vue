			...
			<div class="sh-con-tw ">
				<div class="sh-ri">
					<el-table :data="splittableData" border>
						...
						<el-table-column label="名称/平台SKU" :show-overflow-tooltip="true" prop="sellersku" sortable>
								<template #default="scope">
									<div >{{scope.row.name}}</div>
									<div class="sku">{{scope.row.sellersku}}</div>
								</template>
						</el-table-column>
						<el-table-column label="本地产品SKU"  width="110"   prop="sku"  >
						</el-table-column>
						<el-table-column label="发货数量" width="110" prop="amount" sortable>
							<template #default="scope">
							<div class="table-edit-flex  ">
							<span>{{scope.row.amount}}</span>
							<el-popover 
							v-if="!scope.row.issub"  
							:visible="scope.row.itemshow" 
							placement="top" 
							width="180px" 
							trigger="click" 
							title="拆分发货数量" >
						...
						<el-table-column label="体积" width="90" prop="volumesum" sortable>
							<template #default="scope">
									<div >{{scope.row.volumesum}}<span class="font-extraSmall"> m³</span>
									<div > {{scope.row.volume}} <span class="font-extraSmall"> m³/1</span>
									</div>
									</div>
							</template>
						</el-table-column>
...
<script setup>
	...
	import {formatFloat,dateFormat} from '@/utils/index.js';
			...
			arr.forEach((group,index)=>{
				...
				group.forEach(async (item)=>{
					...
					if(item.checkGroup=="6"){
					    ...
					    obj.name="OD"+nowDate.getFullYear()
						+(nowDate.getMonth()<10?"0":"")
						+(nowDate.getMonth()+1)
						+(nowDate.getDate()<10?"0":"")
						+(nowDate.getDate())
						+"000"+numberindex;
					}
					...
					tab.sellersku=item.sellersku;
					obj.shipnum += item.copshipnum;
					obj.weight += item.weight*item.copshipnum;
					obj.volume += item.volume*item.copshipnum;
					obj.dimweight += item.dimweight*item.copshipnum;
					obj.skunum++
	...
	function show(data,warehouseform){
		var mydata=  JSON.parse(JSON.stringify(data));
		mydata.forEach(row=>{
			row.itemshow=false;
			row.copshipnum=row.amount;
			row.checkGroup='6';
			...
			row.volumesum=formatFloat(row.volume*row.copshipnum);
			row.weightsum=formatFloat(row.weight*row.copshipnum);
		})
		state.splittableData=  JSON.parse(JSON.stringify(mydata));
		state.warehouseform=JSON.parse(JSON.stringify(warehouseform));
		computedCheckGroup();
	...
	async function submitPlan(item){
			...
			item.skulist.forEach(function(spt){
					 var row={};
					 row.amount=spt.quantity;
					 row.fnsku=spt.fnsku;
					 row.id=spt.mid;
					 row.psku=spt.sellersku;
					 tableData.push(row);
			});
		 params=state.warehouseform;
		 params.fromwarehouseid=state.warehouseform.from_warehouseid;
		 params.towarehouseid=state.warehouseform.to_warehouseid;
		 params.arrivaltime=state.warehouseform.arrivalTime;
		 if(params.arrivaltime){
			 params.arrivaltime=dateFormat(params.arrivaltime);
		 }
		 params.groupid=state.warehouseform.groupid;
		 params.country=state.warehouseform.country;
		 params.id="";
		 params.action="submit";
         params.skumapstr=JSON.stringify(tableData);
		 await dispatchApi.saveData(params).then((res)=>{
