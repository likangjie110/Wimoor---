<template>
	<el-dialog v-model="visible" class="invheader" title="盘点单" top="8vh" width="80%">
		<template #title> <el-space><h3>盘点单</h3><el-button @click="showUploadModal">上传</el-button></el-space></template>
		...
		<UploadDialog ref="uploadDialogRef"  :indialog="true"  @upload="uploadItem" /> 
</template>
...
<script setup>
	...
	function getSKUList() {
	 ...
	 function showUploadModal(){
		uploadDialogRef.value.show(state.query.warehouseid);
	 }
	 function uploadItem(formData){  
		formData.append('warehouseid',state.query.warehouseid);
		stocktakingApi.uploadBaseInfoFileSimple(formData).then((res)=>{
			 getRows(res.data);
