...
public class AmzProductSalesPlanServiceImpl extends ServiceImpl<AmzProductSalesPlanMapper, AmzProductSalesPlan> implements IAmzProductSalesPlanService {
	...
	final IAmzInventoryPlanningService iAmzInventoryPlanningService;
/**
 *  * 核心方法-------------------------
 * 处理未来180的销量
 * @param auth 授权
...
public void handlePresaleSKU(AmazonAuthority auth,Marketplace market,
		 ...
		 if(shipcycle>0) {
			  BigDecimal original=new BigDecimal(sumavgsale).divide(new BigDecimal(shipcycle),2,RoundingMode.HALF_UP);
			  BigDecimal roundedUp = original.setScale(0, RoundingMode.UP);
			  sumavgsale=roundedUp.intValue();
	...
	public Map<String,Integer> getSkuInventry(AmazonAuthority auth,String marketplaceid,Map<String,Integer> fulfillMap,String sku){
		...
		for(Map<String, Object> item:listqty) {
			if(item.get("quantity")!=null) {
				...
				if(item.get("formqty")!=null){
					qty=qty+Integer.valueOf(item.get("formqty").toString());
				}
				result.put(item.get("sku").toString(),qty);
			}
			if(fulfillMap!=null&&item.get("fulfillable")!=null) {
		 ...
		 try {
			 ...
			 if(mskulist.size()>0&&dto.getPlantype().equals("ship")) {
				 ...
				 if(dto.getIscheck()!=null) {
					 Map<String,Map<String, Object>> formInboundMap=new HashMap<String,Map<String, Object>>();
					 List<Map<String, Object>> maplist = this.baseMapper.findPlanItemInbound(dto.getGroupid());
					 for (int i = 0; i < maplist.size(); i++) {
						  Map<String, Object> map =maplist.get(i);
						  if(map.get("qty")!=null) {
							  formInboundMap.put(map.get("msku").toString(),map );
						  }
					 }
					
					 List<Map<String, Object>> itemlist = dto.getPlanitem();
					 for(Map<String, Object> planitem:itemlist) {
						 String itemsku=planitem.get("vmsku").toString();
						 Map<String,Object> map=formInboundMap.get(itemsku);
						 if(map!=null&&map.get("qty")!=null) {
							 Integer outone=Integer.parseInt(map.get("qty").toString());
							 Integer amount=Integer.parseInt(planitem.get("marketamount").toString());
							 if(outone!=null) {
								 amount=amount+outone;
								 planitem.put("amount",amount);
								 planitem.put("formqty",outone);
								 planitem.put("formnumber",map.get("num"));
								 formInboundMap.remove(itemsku);
							 }
						 }
						
					 }
//					 for(Entry<String, Map<String, Object>> entry:formInboundMap.entrySet()) {
//						 Map<String,Object> map=entry.getValue();
//						 Map<String,Object> newitem=new HashMap<String,Object>();
//						 newitem.put("vmsku", entry.getKey());
//						 Integer outone=Integer.parseInt(map.get("qty").toString());
//						 newitem.put("amount", outone);
//						 newitem.put("formqty",outone);
//						 newitem.put("formnumber",map.get("num"));
//						 itemlist.add(newitem);
//						 dto.getMskulist().add(entry.getKey());
//					 }
...
public void setShipRecord(Map<String,Object> item,String shopid,String groupid,String marketplaceid,String sku,int puOnDay) {
    ...
    public void handItem(Map<String, Object> item,Map<String,Map<String,Object>> planmap,PlanDetailDTO dto) {
		...
		fbaquantity=fbaquantity+(item.get("formqty")!=null?Integer.parseInt(item.get("formqty").toString()):0);
