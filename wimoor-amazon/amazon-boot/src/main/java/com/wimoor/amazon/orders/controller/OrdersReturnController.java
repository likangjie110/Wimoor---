...
public class OrdersReturnController{
	...
	@ApiOperation("订单退货列表")
	@PostMapping("/returnSKUlist")
	public  Result<IPage<Map<String,Object>>> returnListBySKUAction(
			@RequestBody AmazonOrdersReturnDTO condition){
		UserInfo userinfo = UserInfoContext.get();
		if(userinfo.isLimit(UserLimitDataType.operations)) {
			condition.setOwner(userinfo.getId());
		}
		return Result.success(orderReturnService.selectReturnsListBySKU(condition));
	}
	@ApiOperation("订单退货列表")
	@PostMapping("/selectReturnsSummaryByDay")
	public  Result<Map<String,Object>> selectReturnsSummaryByDayAction(
			@RequestBody AmazonOrdersReturnDTO condition){
		UserInfo userinfo = UserInfoContext.get();
		if(userinfo.isLimit(UserLimitDataType.operations)) {
			condition.setOwner(userinfo.getId());
		}
		return Result.success(orderReturnService.selectReturnsSummaryByDay(condition));
	}
	@ApiOperation("订单退货列表")
	@PostMapping("/selectReturnsSummaryByType")
	public  Result<List<Map<String,Object>>> selectReturnsSummaryByTypeAction(
			@RequestBody AmazonOrdersReturnDTO condition){
		UserInfo userinfo = UserInfoContext.get();
		if(userinfo.isLimit(UserLimitDataType.operations)) {
			condition.setOwner(userinfo.getId());
		}
		return Result.success(orderReturnService.selectReturnsSummaryByType(condition));
	}
