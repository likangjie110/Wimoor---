...
public class ShipInboundTransportationServiceImpl implements IShipInboundTransportationService {
	...
	final IShipV2InboundplanPalletService shipV2InboundplanPalletService;
	@Override
	public ShipInboundOperation generateTransportationOptions(ShipInboundPlan plan) {
		// TODO Auto-generated method stub
		shipInboundPlanTransportationOptionsMapper.delete(new LambdaQueryWrapper<ShipInboundPlanTransportationOptions>()
				.eq(ShipInboundPlanTransportationOptions::getFormid,plan.getId())
				.eq(ShipInboundPlanTransportationOptions::getPlacementOptionId, plan.getPlacementOptionId()));
...
//			if("LTL".equals(plan.getTranstyle())){
//				List<ShipV2InboundplanPallet> palletList =shipV2InboundplanPalletService.lambdaQuery()
//						.eq(ShipV2InboundplanPallet::getShipmentid, shipmentid)
//						.eq(ShipV2InboundplanPallet::getFormid, plan.getId())
//						.list();
//				List<PalletInput> pallets=new ArrayList<PalletInput>();
//				for (ShipV2InboundplanPallet pallet : palletList) {
//					PalletInput e=new PalletInput();
//					Dimensions dim=new Dimensions();
//					dim.setHeight(pallet.getHeight());
//					dim.setWidth(pallet.getWidth());
//					dim.setLength(pallet.getLength());
//					dim.setUnitOfMeasurement(UnitOfMeasurement.valueOf(pallet.getDimUnit().toUpperCase()));
//					e.setQuantity(pallet.getNum());
//					e.setStackability(Stackability.STACKABLE);
//					Weight weight=new Weight();
//					weight.setValue(pallet.getWeight());
//					weight.setUnit(UnitOfWeight.valueOf(pallet.getWeightUnit().toUpperCase()));
//					e.setWeight(weight);
//					e.setDimensions(dim);
//					pallets.add(e);
//				}
...
//			}
	...
	public ListTransportationOptionsResponse listTransportationOptions(TransportationDTO dto) {
	    ...
	    if(StrUtil.isBlank(token)){
  			LambdaQueryWrapper<ShipInboundPlanTransportationOptions> query=new LambdaQueryWrapper<ShipInboundPlanTransportationOptions>();
			query.eq(ShipInboundPlanTransportationOptions::getFormid, plan.getId());
			query.eq(ShipInboundPlanTransportationOptions::getPlacementOptionId, dto.getPlacementOptionId());
			if(StrUtil.isNotBlank(dto.getShipmentid())){
			...
			if(translist!=null&&translist.size()>0){
				...
				if(StrUtil.isBlank(dto.getShipmentid())) {
					this.shipInboundShipmentService.lambdaUpdate()
							.eq(ShipInboundShipment::getFormid, plan.getId())
		...
		try{
			...
			for(TransportationOption option:response.getTransportationOptions()){
				...
				if(StrUtil.isNotBlank(dto.getPlacementOptionId())){
					o.setPlacementOptionId(dto.getPlacementOptionId());
				}else{
