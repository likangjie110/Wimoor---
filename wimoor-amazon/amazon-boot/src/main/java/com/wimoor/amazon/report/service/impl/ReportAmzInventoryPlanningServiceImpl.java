		...
		try {
			while ((line = br.readLine()) != null) {
				...
				if (lineNumber == 0) {
					...
					e.setFbaInventoryLevelHealthStatus(GeneralUtil.getStrValue(info, titleList, "fba-inventory-level-health-status"));
					e.setFbaMinimumInventoryLevel(GeneralUtil.getStrValue(info, titleList, "fba-minimum-inventory-level"));
					e.setHistoricalDaysOfSupply(GeneralUtil.getStrValue(info, titleList, "historical-days-of-supply"));
					if(lineNumber==1) {
