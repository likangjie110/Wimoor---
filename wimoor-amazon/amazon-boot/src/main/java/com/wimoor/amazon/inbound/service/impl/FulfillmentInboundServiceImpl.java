...
public class FulfillmentInboundServiceImpl implements IFulfillmentInboundService {
	...
	public List<ShipInboundShipment> createInboundShipmentPlan(AmazonAuthority auth,	Marketplace market ,ShipInboundPlan inplan) {
	   ...
	   public void asyncNeedShipment() {
		   List<Map<String, Object>> list = shipInboundShipmentMapper.getNeedAsyncShipment();
		   for (Map<String, Object> item : list) {
			   String shipmentids =   item.get("shipmentids").toString();
			   String amazonauthid =  item.get("amazonauthid").toString();
			   String marketplaceid =  item.get("marketplaceid").toString();
				   AmazonAuthority amazonAuthority = iAmazonAuthorityService.getById(amazonauthid);
				   if (amazonAuthority != null) {
					   Marketplace marketplace = marketplaceService.getById(marketplaceid);
					   try{
						   this.listShipmentAsync(amazonAuthority, marketplace, Arrays.asList(shipmentids.split(",")));
					   }catch(Exception e) {
						   e.printStackTrace();
					   }
				   }
	...
	public String getLabelDownloadUR(AmazonAuthority amazonAuthority,String shipmentid,Integer boxnum,String pagetype,String labelType,String pannum){
		 FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
		 try {
			...
			if("PALLET".equals(labelType)) {
				...
				response=api.getLabels(shipmentid,pagetype , labelType, boxnum, null, pansize, boxnum, null);
			}else {
				response=api.getLabels(shipmentid,pagetype , labelType, boxnum, null, null, boxnum, null);
			}
	...
	public void listShipmentAsync(AmazonAuthority auth,Marketplace market){
	     ...
	     try {
			   ...
			   shipmentStatusList.add("CHECKED_IN");
			   shipmentStatusList.add("WORKING");
			   ApiCallback<GetShipmentsResponse> callback=new ApiCallbackGetShipments(this,iShipInboundShipmentService,auth,market);
	...
	private void listShipmentItemsAsync(AmazonAuthority amazonAuthority, Marketplace market) {
		...
		FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
		  try {
	    	 ...
	    	 cstart.add(Calendar.DATE, -3);
	    	 Calendar cend=Calendar.getInstance();
			 ApiCallback<GetShipmentItemsResponse> callback=new ApiCallbackGetShipmentItems(this,iShipInboundItemService,amazonAuthority,market,null);
	...
	private void getItemsByShipmentIdAsync(AmazonAuthority amazonAuthority, Marketplace market,ShipInboundShipment shipment) {
		...
		FbaInboundApi api = apiBuildService.getInboundApi(amazonAuthority);
		  try {
			 ApiCallback<GetShipmentItemsResponse> callback=new ApiCallbackGetShipmentItems(this,iShipInboundItemService,amazonAuthority,market,shipment);
	...
	public void handlerItemResult(AmazonAuthority auth, Marketplace market, GetShipmentItemsResponse result, ShipInboundShipment param_shipment,boolean needshipqty) {
		...
		if(itemResult.getNextToken()!=null) {
			FbaInboundApi api = apiBuildService.getInboundApi(auth);
			 ApiCallback<GetShipmentItemsResponse> callback=new ApiCallbackGetShipmentItems(this,iShipInboundItemService,auth,market,param_shipment);
			try {
