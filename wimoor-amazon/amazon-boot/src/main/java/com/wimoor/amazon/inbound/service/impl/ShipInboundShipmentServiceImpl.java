...
public class ShipInboundShipmentServiceImpl extends  ServiceImpl<ShipInboundShipmentMapper,ShipInboundShipment> implements IShipInboundShipmentService { 
	...
	final com.wimoor.amazon.inboundV2.service.IShipInboundShipmentService shipInboundShipmentV2Service;
	@Resource
...
public String createInboundShipment(ShipInboundShipment shipment) {
	...
	String mshipmentid="";
	if(inplan.NeedAmz()) {
		mshipmentid=iFulfillmentInboundService.createInboundShipment(auth, market, inplan, shipment);
	}else {
		mshipmentid=shipment.getShipmentid();
	}
...
private void updateItemList(AmazonAuthority auth,Marketplace market,ShipInboundPlan inplan,List<ShipInboundItem> itemlist) {
...
public String updateInboundShipment(ShipInboundShipment shipment, String actiontype) {
	...
	this.updateById(shipment);
	updateItemList(auth,market,inplan,shipment.getItemList());
	return shipmentid;
}
...
public String getLabelUrl(String shipmentid,String pagetype,String labelType,String pannum) {
	...
	if(shipment.getStatus()>=2 &&shipment.getStatus()<=4) {
		List<Map<String, Object>> boxlist = shipInboundBoxMapper.findShipInboundBox(shipmentid);
		if(shipment.getSubmissionid()==null) {
			throw new BizException("您还没有提交箱子信息，无法操作");
		}
		if(shipment.getBoxnum()==null||boxlist==null||shipment.getBoxnum()!=boxlist.size()) {
			throw new BizException("箱子信息与存储的记录不匹配");
		}
		shipment.setStatus(4);
		shipment.setStatus4date(new Date());
		this.updateById(shipment);
		this.shipInboundShipmentRecordService.saveRecord(shipment);
	}
	return iFulfillmentInboundService.getLabelDownloadUR(auth, shipment.getShipmentid(),shipment.getBoxnum(), pagetype,labelType,pannum);
}
...
public ShipInboundShipmenSummarytVo getUnSyncShipment(String groupid, String marketplaceid, String shipmentid, String warehouseid) {
    ...
    public Integer refreshShipmentRec(String shipmentid) {
    	...
    	ShipInboundShipmentDTO ship = this.syncShipment(plan.getAmazongroupid(),plan.getMarketplaceid(),shipmentid,null);
        Integer result=0;
		for( ShipInboundItemDTO  item:ship.getItemList()) {
	...
	public void saveTransTrance(UserInfo user,ShipInboundShipment shipment,String carrier,String transtyle,List<Map<String, Object>> boxinfo){
     ...
     public void handleShipmentV1Trace(ShipInboundShipment shipment,ShipInboundshipmentTraceupload item){
	...
	public void saveTransTrance(){
		...
		for(ShipInboundshipmentTraceupload item:list) {
			try{
				com.wimoor.amazon.inboundV2.pojo.entity.ShipInboundShipment shipmentV2 = shipInboundShipmentV2Service.lambdaQuery().eq(com.wimoor.amazon.inboundV2.pojo.entity.ShipInboundShipment::getShipmentConfirmationId, item.getShipmentid()).one();
				 if(shipmentV2!=null){
					 shipInboundShipmentV2Service.handleShipmentV2Trace(shipmentV2);
				 }else{
					 ShipInboundShipment shipment = this.baseMapper.selectById(item.getShipmentid());
					 if(shipment!=null){
						 handleShipmentV1Trace(shipment, item);
					 }
				 }
				 item.setStatus(1);
			}catch(BizException e) {
	...
	public Map<String, Object> getTransChannelInfo(String id) {
		return this.erpClientOneFeign.getShipTransChannelDetial(id);
	}

	@Override
	public List<Map<String, Object>> getBoxNum(String shopid) {
		return this.shipInboundBoxMapper.findBoxNum(shopid);
	}
	...
	public String saveMarkshiped(UserInfo user, String shipmentid,String operateType)   {
		ShipInboundShipment shipment = this.getById(shipmentid);
		ShipInboundPlan plan = shipInboundPlanService.getById(shipment.getInboundplanid());
		int oldstatus=shipment.getStatus();
		if(plan.NeedAmz()==false) {
        ...
        try {
        	  ...
        	  boolean isupdate =this.lambdaUpdate().set(ShipInboundShipment::getStatus, shipment.getStatus())
	        				...
	        				.set(ShipInboundShipment::getOperator, shipment.getOperator())
	        				.set(ShipInboundShipment::getOpttime, shipment.getOpttime())
	        				.set(ShipInboundShipment::getShipmentstatus, shipment.getShipmentstatus())
	        				.set(ShipInboundShipment::getShipedDate, shipment.getShipedDate())
	        				.eq(ShipInboundShipment::getShipmentid, shipment.getShipmentid()).update();
				if (isupdate==true) {
				    shipInboundShipmentRecordService.saveRecord(shipment);
				}
        		if(plan.NeedInv()) {
        			if(oldstatus<5&&oldstatus>1) {
	...
	@Override
	public int saveTransData(UserInfo user,String shipmentid, String company, String channel,String transtyle,String transtype) {
		int result=0;
		ShipInboundShipment shipment = this.baseMapper.selectById(shipmentid);
		...
		if (list.size() > 0) {
			...
			item.setTranstype(new BigInteger(transtype));
			item.setOperator(user.getId());
			item.setOpttime(new Date());
			...
			ship.setTranstype(new BigInteger(transtype));
