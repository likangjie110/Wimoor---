...
public class AmazonAuthority  extends  BaseEntity  implements RateLimitConfiguration{
	...
	/*
	 * @TableField(value = "access_key_id") private String accessKeyId;
	 * 
	 * @TableField(value = "secret_key") private String secretKey;
	 * @TableField(value = "role_arn") private String roleArn;
	 */
	...
	public AmzAuthApiTimelimit getApiRateLimit() {
		...
		AmzAuthApiTimelimit limit =null;
		if(this.getUseApi()!=null){
			IAmzAuthApiTimelimitService amzAuthApiTimelimitService=SpringUtil.getBean("amzAuthApiTimelimitService");
			limit = amzAuthApiTimelimitService.getApiLimit(this.getId(), this.getUseApi());
		}
	...
	public void setApiStatus(LWAException e) {
		// TODO Auto-generated method stub
		if(e!=null&&e.getErrorMessage()!=null&&e.getErrorMessage().contains("invalid grant parameter : refresh_token")) {
			IAmazonAuthorityService iAmazonAuthorityService=SpringUtil.getBean(AmazonAuthorityServiceImpl.class);
			this.status="invalid";
			this.statusupdate=new Date();
			iAmazonAuthorityService.updateById(this);
		}
	}
