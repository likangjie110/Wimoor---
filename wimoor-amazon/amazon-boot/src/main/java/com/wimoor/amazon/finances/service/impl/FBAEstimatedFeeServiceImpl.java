...
public class FBAEstimatedFeeServiceImpl extends ServiceImpl<FBAEstimatedFeeMapper, FBAEstimatedFee> implements IFBAEstimatedFeeService {
	...
	public FBAEstimatedFee getOneBySku(String sku, String asin, String amazonauthid, String marketplaceid) {
		...
		FBAEstimatedFee one = this.getOne(queryFbaFee);
		if(one!=null){
			if(one.getEstimatedFeeTotal()!=null&&one.getEstimatedReferralFeePerUnit()!=null&&one.getEstimatedFeeTotal().compareTo(one.getEstimatedReferralFeePerUnit())==0) {
				 if(one.getExpectedEfnFulfilmentFeePerUnitUk()!=null) {
					 one.setEstimatedFeeTotal(one.getEstimatedFeeTotal().add(one.getExpectedEfnFulfilmentFeePerUnitUk()));
				 }
				else if(one.getExpectedEfnFulfilmentFeePerUnitDe()!=null) {
					one.setEstimatedFeeTotal(one.getEstimatedFeeTotal().add(one.getExpectedEfnFulfilmentFeePerUnitDe()));
				}
			}
		}
		return one;
