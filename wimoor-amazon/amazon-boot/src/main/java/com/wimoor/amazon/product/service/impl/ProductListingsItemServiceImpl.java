...
public class ProductListingsItemServiceImpl implements IProductListingsItemService {
	...
	public Item captureListMatchingProduct(AmazonAuthority amazonAuthority, String sku, List<String> marketplaces,String issueLocale, List<String> includedData ) {
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new BizException("接口调用错误："+e.getErrorMessage());
		}
	}
	...
	@Override
	public ListingsItemSubmissionResponse putListingsItem(ProductListingPushDTO dto) {
		// TODO Auto-generated method stub
		AmazonAuthority amazonAuthority = amazonAuthorityService.getById(dto.getAmazonauthid());
		ListingsApi api = apiBuildService.getProductApi(amazonAuthority);
		try {
			ListingsItemPutRequest request=new ListingsItemPutRequest();
			request.setAttributes(dto.getAttributes());
			request.setProductType(dto.getProductType());
			request.setRequirements(RequirementsEnum.LISTING);
            return api.putListingsItem(amazonAuthority.getSellerid(), dto.getSku(), dto.getMarketplaceids(),request , dto.getIssueLocale());
		} catch (ApiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (LWAException e) {
		...
		public Object deleteAsin(ProductListingItemDTO dto) {
			...
			try {
				...
				if(opt!=null) {
					...
					opt=new ProductInOpt();
					opt.setDisable(true);
					opt.setPid(new BigInteger(infoone.getId()));
					productInOptMapper.insert(opt);
				}
			}
			return response;
		...
		public ProductInfo saveAsin(ProductListingItemDTO dto) {
			...
			if(oldinfo!=null&&oldinfo.size()>0) {
				...
				info.setId(amazonAuthorityService.getUUID());
		...
		public  ListingsItemSubmissionResponse changePriceSku(ProductListingItemDTO dto) {
				...
				List<ProductInfo> info = iProductInfoService.selectBySku(dto.getSku(), market.getMarketplaceid(), amazonAuthority.getId());
				if(info==null||info.size()==0) {
					throw new BizException("未找到对应产品");
				}
	...
	@Override
	public ListingsItemSubmissionResponse patchListingsItem(ProductListingPushDTO dto) {
		AmazonAuthority amazonAuthority = amazonAuthorityService.getById(dto.getAmazonauthid());
		Marketplace market = iMarketplaceService.getById(dto.getMarketplaceids().get(0));
		List<ProductInfo> info = iProductInfoService.selectBySku(dto.getSku(), market.getMarketplaceid(), amazonAuthority.getId());
		ListingsApi api = apiBuildService.getProductApi(amazonAuthority);
        try {
            return api.patchListingsItem(amazonAuthority.getSellerid(), dto.getSku(), dto.getMarketplaceids(), dto.getPatchBody(), dto.getIssueLocale());
		...
		public  ListingsItemSubmissionResponse changePriceSku(String userid,AmazonAuthority amazonAuthority,Marketplace market,ProductInfo info,ProductInOpt opt,String newprice,String fromDate,String toDate) {
				...
				try {
					...
					if(StrUtil.isNotEmpty(fromDate)) {
						...
						String ofromDate = GeneralUtil.getCountryDateToUTC(fromDate,market.getMarket());
						String otoDate = GeneralUtil.getCountryDateToUTC(toDate,market.getMarket());
						itemAttribute.setDiscounted_price_start_at(ofromDate);
						itemAttribute.setDiscounted_price_end_at(otoDate);
					}else {
						Money price=new Money();
