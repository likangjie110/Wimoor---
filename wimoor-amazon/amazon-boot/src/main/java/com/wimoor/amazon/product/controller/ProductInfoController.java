...
public class ProductInfoController {
	...
	ErpClientOneFeignManager erpClientOneFeign;
		...
		public Result<IPage<AmzProductListVo>> productListAction(@RequestBody ProductListQuery query) {
			...
			try {
				if(StrUtil.isNotBlank(query.getIsparent())) {
					IPage<AmzProductListVo> list = iProductInfoService.getListParentByUser(userinfo,query, parameter);
		...
		@PostMapping("/productParentListDetail")
		public Result<List<AmzProductListVo>> productParentListDetail(@RequestBody ProductListQuery query) {
			String search =query.getSearch();
			String searchtype = query.getSearchtype();
			Map<String, Object> parameter = new HashMap<String, Object>();
			parameter.put("searchtype", searchtype);
			...
			try {
					List<AmzProductListVo> list = iProductInfoService.getListDetailByUser(userinfo,query, parameter);
					return Result.success(list);
			}catch(Exception e) {
				e.printStackTrace();
			}
