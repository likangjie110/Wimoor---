...
public class ProductInfoServiceImpl extends ServiceImpl<ProductInfoMapper, ProductInfo> implements IProductInfoService {
    ...
    public List<String> getPidListByTagList(List<String> taglist,Object shopid,Object amazonAuthId,Object groupid,Object groupList,Object marketplace,Boolean withouttag) {
    	if(taglist!=null && taglist.size()>0) {
			...
			String without=withouttag?"true":"false";
			HashMap<String,Object> param=new HashMap<String,Object>();
			try {
				if(taglist!=null&&taglist.size()>0) {
					results = erpClientOneFeign.getMskuByTagList(without,taglist);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	@Override
	public List<AmzProductListVo> getListDetailByUser(UserInfo userinfo, ProductListQuery query, Map<String, Object> parameter) {
		// TODO Auto-generated method stub
		parameter.put("shopid", userinfo.getCompanyid());
		List<String> taglist = query.getTaglist();//123,456,122,
		if(parameter.get("groupList")!=null) {
		...
		List<AmzProductListVo> productList = this.baseMapper.selectDetialByAuth(parameter);
		if(productList!=null  && productList.size()>0) {
			handleProductListVo(userinfo,parameter,productList);
		}
		return productList;
	}
    public void handleProductListVo(UserInfo userinfo,Map<String, Object> parameter,List<AmzProductListVo> list) {
    	List<AmzProductListVo> euVolist=new LinkedList<AmzProductListVo>();
		...
		for(AmzProductListVo item:list) {
			...
			if(item.getRegion().equals("EU")) {
				 haseu=true;
				 euVolist.add(item);
				 LambdaQueryWrapper<AmzInventoryCountryReport> orone = queryWrapper2.or();
				 orone.eq(AmzInventoryCountryReport::getAuthid, item.getAmazonAuthId());
				 orone.eq(AmzInventoryCountryReport::getSku, item.getSku());
			}
			mskuList.add(item.getMsku());
			Map<String, Object> myparameter = new HashMap<String, Object>();
			String sku = item.getSku();
			String marketplaceid = item.getMarketplaceid();
			String groupid = item.getGroupid();
			myparameter.put("sku", sku);
			myparameter.put("marketplaceid", marketplaceid);
			myparameter.put("groupid", groupid);
			myparameter.put("shopid", userinfo.getCompanyid());
		...
		if(haseu) {
			...
			if(eulist!=null && eulist.size()>0) {
				Map<String,List<AmzInventoryCountryReport>>  countryinv=new HashMap<String,List<AmzInventoryCountryReport>>();
				for (int i = 0; i < eulist.size(); i++) {
					AmzInventoryCountryReport items = eulist.get(i);
					String ckey=items.getAuthid().toString()+items.getSku();
					List<AmzInventoryCountryReport> clist = countryinv.get(ckey);
					if(clist==null) {
						clist=new LinkedList<AmzInventoryCountryReport>();
					}
					clist.add(items);
					countryinv.put(ckey, clist);
				}
				for(AmzProductListVo item:euVolist) {
					String ckey=item.getAmazonAuthId().toString()+item.getSku();
					List<AmzInventoryCountryReport> clist = countryinv.get(ckey);
					if(clist!=null&&clist.size()>0) {
						int sum=0;
						for(AmzInventoryCountryReport items:clist) {
		...
		try {
         ...
         getWeekOtherCostForSKU(userinfo, list,skulist,mskuList);
    }
    
    
	public IPage<AmzProductListVo> getListParentByUser(UserInfo userinfo, ProductListQuery query, Map<String, Object> parameter) {
		// TODO Auto-generated method stub
		parameter.put("shopid", userinfo.getCompanyid());
		List<String> taglist = query.getTaglist();//123,456,122,
		if(parameter.get("groupList")!=null) {
		...
		Set<String> set=new HashSet<String>(Arrays.asList("amazonAuthId","marketplaceid","parentAsin","name","image","landed_amount","landed_currency","changeprice",
				"averageSalesDay","averageSalesWeek","averageSalesMonth","listing_currency","sumweek","summonth","orderweek","sales15","ordermonth","sessions",
				"sessionrate","unitsordered7","advimpr","advsales","advcpc","advclick","advspc","advspend","advctr","advacos","afn_fulfillable_quantity",
				"buybox","prorate","profitrate","profitall","proprate","marketplacename","pointName","groupname","newprorate","country","marketindex"
				));
		 
		if(!set.contains(query.getSort()) ) {
			query.setSort("sku,marketindex");
		}
		IPage<AmzProductListVo> productList = this.baseMapper.selectParentDetialByAuth(query.getPage(),parameter);
		 for(AmzProductListVo item:productList.getRecords()) {
			 parameter.put("parentasin", item.getParentAsin());
			 parameter.put("amazonAuthId", item.getAmazonAuthId());
			 parameter.put("marketplace", item.getMarketplaceid());
			 parameter.put("isparent", null);
			 //List<AmzProductListVo> children = this.baseMapper.selectDetialByAuth(parameter);
			 //this.handleProductListVo(userinfo, parameter, children);
	...
	public void getWeekOtherCostForSKU(UserInfo user, List<AmzProductListVo> productList, List<Map<String, Object>> skulist, List<String> mskuList) {
		if (productList != null) {
			List<Map<String, Object>> allItemList = null;
			if(skulist!=null&&skulist.size()>0) {
				allItemList=iAmzFinUserItemDataService.getFinDataLastWeek(skulist);
			}
			Map<String, Map<String, Object>> mskuDimInfoMap=null;
			if(mskuList!=null&&mskuList.size()>0) {
    	    	 mskuDimInfoMap = erpClientOneFeign.getMaterialBySKUAction(user.getCompanyid(),mskuList);
    	    }
			...
			for (AmzProductListVo  item : productList) {
				...
				info.setMarketplaceid(item.getMarketplaceid());
				AmazonAuthority auth = amazonAuthorityService.getById(info.getAmazonAuthId());
				ProductInOpt opt =null;
				if(item.getProfitid()!=null){
					opt =new ProductInOpt();
					opt.setPid(new BigInteger(item.getId()));
					opt.setProfitid(new BigInteger(item.getProfitid()));
				}
		        parameter.put("msku",item.getMsku());
		        parameter.put("productInProfit",null);
		        parameter.put("info",info);
		        parameter.put("proopt",opt);
		        parameter.put("auth",auth);
				try {
		    	    
		    	    if(mskuDimInfoMap!=null &&mskuDimInfoMap.get(item.getMsku().trim())!=null) {
		    	    	parameter.put("mskuDim",mskuDimInfoMap.get(item.getMsku().trim()));
		    	    }
	...
	public Map<String, Object> updateProductPrice(UserInfo user, Map<String, Object> map, String ftype) {
		...
		public void showProfitDetial(Map<String, Object> map) {
			...
			ProductInProfit productInProfit =map.get("productInProfit")!=null?(ProductInProfit) map.get("productInProfit"):null;
			ProductInfo info = (ProductInfo) map.get("info");
			ProductInOpt proopt =map.get("proopt")!=null? (ProductInOpt) map.get("proopt"):null;
			AmazonAuthority auth = (AmazonAuthority) map.get("auth");
			String msku=map.get("msku").toString().trim();
		    ...
		    String costDetailStr = null;
				...
				if(map.get("mskuDim")!=null) {
					Map<String, Object> data = (Map<String, Object>) map.get("mskuDim");
	    ...
	    public void setLocalValue(Map<String, Object> map,String pid ,String msku) {
			...
			if(transFeeMap!=null&&transFeeMap.get("skufeeavg")!=null) {
				...
				Object mcurrShipmentFee = transFeeMap.get("skufeeavg");
		...
		public ProductInProfit getInPorfit(String pid) {
			// TODO Auto-generated method stub
			return this.productInProfitMapper.selectById(pid);
		}
}
