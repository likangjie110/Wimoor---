...
public class AmazonSellerMarketController {
    ...
    @CacheEvict(value = "authmarketplaceListCache",allEntries = true)
    public Result<?> statusChange(String sellerid,String marketplaceid,String status) {
    	 AmazonSellerMarket one=iAmazonSellerMarketService.lambdaQuery()
    	.eq(AmazonSellerMarket::getSellerid,sellerid)
    	.eq(AmazonSellerMarket::getMarketplaceId, marketplaceid).one();
        if(one!=null) {
        	one.setDisable("true".equals(status));
        	iAmazonSellerMarketService.lambdaUpdate().set(AmazonSellerMarket::getDisable, one.getDisable())
        			.eq(AmazonSellerMarket::getSellerid, sellerid)
        			.eq(AmazonSellerMarket::getMarketplaceId, marketplaceid).update();
        }
        return Result.judge(true);
    }
}
