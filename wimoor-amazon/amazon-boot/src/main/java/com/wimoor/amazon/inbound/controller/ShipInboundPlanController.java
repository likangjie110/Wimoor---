	...
	public ShipInboundPlan saveInboundPlan(@ApiParam("发货计划")@RequestBody ShipInboundPlanDTO inplan){
		  try {
				...
				for(ShipInboundItemVo itemvo:list) {
					...
					if(inplan.getArecasesrequired()) {
						item.setQuantityincase(itemvo.getBoxnum());
					}else {
						item.setQuantityincase(null);
					}
					item.setQuantityshipped(itemvo.getQuantity());
					item.setSellersku(itemvo.getSellersku());
					itemlist.add(item);
				}
				...
				if(inplanparam.getInvtype()==null) {
					inplanparam.setInvtype(0);
				}
	...
	public Result<IPage<Map<String, Object>>> getShipRecordAction(String groupid,String marketplaceid,String sku)   {
		...
		if(shipRecord==null||shipRecord.size()<=0) {
			...
			for(int i=0;i<shipRecord.size();i++) {
				map.put(shipRecord.get(i).get("shipmentid").toString(),shipRecord.get(i));
			}
			for(int i=0;i<shipBadRecord.size();i++) {
				if(map.get(shipRecord.get(i).get("shipmentid").toString())!=null) {
					map.get(shipRecord.get(i).get("shipmentid").toString()).put("isbad",1);
				}else{
					shipRecord.add(shipBadRecord.get(i));
				}
			}
		}
