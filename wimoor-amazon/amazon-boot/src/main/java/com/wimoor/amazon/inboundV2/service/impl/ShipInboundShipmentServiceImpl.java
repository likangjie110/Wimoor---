...
import com.wimoor.quote.api.dto.ShipmentBoxDTO;
import com.wimoor.quote.api.dto.ShipmentDestinationAddressDTO;
import com.wimoor.quote.api.dto.ShipmentItemDTO;
...
public String saveShipment(ShipInboundPlan inplan, List<String> shipmentids) {
	...
	for(String shipmentid:shipmentids) {
		...
		try{
			shipment=asyncShipmentList(auth,inplan,shipmentid);
		}catch(Exception e){
			throw new BizException("同步货件"+shipmentid+"失败！",e);
		}
        try{
			...
			ShipInboundShipment mshipment = this.getById(shipmentid);
			if(StrUtil.isNotBlank(mshipment.getRemark())&&mshipment.getRemark().contains("[log:同步产品列表失败]")){
				mshipment.setRemark(mshipment.getRemark().replace("[log:同步产品列表失败]",""));
			}
		}catch(Exception e){
			ShipInboundShipment mshipment = this.getById(shipmentid);
			if(StrUtil.isNotBlank(mshipment.getRemark())&&!mshipment.getRemark().contains("[log:同步产品列表失败]")){
				mshipment.setRemark(mshipment.getRemark()+"[log:同步产品列表失败]");
				this.updateById(mshipment);
			}else if(StrUtil.isBlank(mshipment.getRemark())){
				mshipment.setRemark("[log:同步产品列表失败]");
				this.updateById(mshipment);
			}
		}
       try{
		   ...
		   ShipInboundShipment mshipment = this.getById(shipmentid);
		   if(StrUtil.isNotBlank(mshipment.getRemark())&&mshipment.getRemark().contains("[log:同步箱子详情失败]")){
			   mshipment.setRemark(mshipment.getRemark().replace("[log:同步箱子详情失败]",""));
		   }
	   }catch(Exception e){
		   ShipInboundShipment mshipment = this.getById(shipmentid);
		   if(StrUtil.isNotBlank(mshipment.getRemark())&&!mshipment.getRemark().contains("[log:同步箱子详情失败]")){
			   mshipment.setRemark(mshipment.getRemark()+"[log:同步箱子详情失败]");
			   this.updateById(mshipment);
		   }else if(StrUtil.isBlank(mshipment.getRemark())){
			   mshipment.setRemark("[log:同步箱子详情失败]");
			   this.updateById(mshipment);
		   }
	   }
...
private void asyncShipmentBox(ShipmentItemsDTO boxdto,Shipment shipment){
	...
	for(Box box:boxs.getBoxes()) {
		...
		if(!entity.getUnit().equals(UnitOfMeasurement.CM.getValue())){
			String unit="";
			if(box.getDimensions().getUnitOfMeasurement().getValue().equals(UnitOfMeasurement.IN.getValue())){
				unit=InputDimensions.unit_in;
			}
			String wunit="";
			if(box.getWeight().getUnit().getValue().equals(UnitOfWeight.LB.getValue())){
				wunit=InputDimensions.unit_lb;
			}
			InputDimensions dims=new InputDimensions(entity.getLength(), entity.getWidth(), entity.getHeight(), unit,entity.getWeight(),wunit);
		    entity.setLength(dims.getLength(InputDimensions.unit_cm).getValue());
		    entity.setWidth(dims.getWidth(InputDimensions.unit_cm).getValue());
		    entity.setHeight(dims.getHeight(InputDimensions.unit_cm).getValue());
			entity.setUnit("CM");
		    entity.setWeight(dims.getWeight(InputDimensions.unit_kg).getValue());
			entity.setWunit("KG");
		}
...
private void asyncShipmentItem(ShipmentItemsDTO dto){
	...
	LambdaQueryWrapper<ShipInboundShipmentItem> queryShipmentItemWrapper=new LambdaQueryWrapper<ShipInboundShipmentItem>();
	queryShipmentItemWrapper.eq(ShipInboundShipmentItem::getShipmentid, dto.getShipmentid());
	List<ShipInboundShipmentItem> oldList = shipInboundShipmentItemV2Mapper.selectList(queryShipmentItemWrapper);
	Map<String,ShipInboundShipmentItem> oldMap=new HashMap<String,ShipInboundShipmentItem>();
	for(ShipInboundShipmentItem item:oldList) {
	...
	for(Item item:items.getItems()) {
		...
		ShipInboundShipmentItem oldone =oldMap.get(entity.getSku());
		if(oldone!=null) {
			...
			oldMap.remove(entity.getSku());
		}else {
			shipInboundShipmentItemV2Mapper.insert(entity);
		}
	}
	if(oldMap.size()>0) {
		for(Entry<String, ShipInboundShipmentItem> entry:oldMap.entrySet()) {
			shipInboundShipmentItemV2Mapper.deleteById(entry.getValue().getId());
		}
	}
}
...
public void setTranStyle(ShipInboundShipment ship){
	...
	public Shipment asyncShipmentList(AmazonAuthority auth,ShipInboundPlan inplan,String shipmentid) {
		...
		if(oldOne==null) {
			...
			ship.setTranstype(inplan.getTranstype());
			if(inplan.getSubmitbox()) {
				ship.setStatus(4);
			}else {
				ship.setStatus(3);
...
public ShipInboundOperation handleUpdateTraceInfo(ShipInboundShipment ship,List<ShipInboundShipmentBox> boxinfo){
	...
	if (ship.getTranstyle()!=null&&ship.getTranstyle().equals("LTL")) {
...
public void updateFeeByShipment(List<ShipInboundShipmentItem> list) {
	...
	public void toQuote(UserInfo user,String token,String shipmentId) {
		...
		if(shipment.getIsquote()==null|| !shipment.getIsquote()) {
			...
			dto.setToken(token);
			com.wimoor.quote.api.dto.ShipmentDTO shipmentdto=new com.wimoor.quote.api.dto.ShipmentDTO();
			...
			List<ShipmentBoxDTO> dtoBoxlist=new ArrayList<ShipmentBoxDTO>();
			BigDecimal weight=BigDecimal.ZERO;
			BigDecimal volume=BigDecimal.ZERO;
			for(ShipInboundShipmentBox box:boxlist) {
				ShipmentBoxDTO dtoBox = new ShipmentBoxDTO();
			...
			List<Map<String, Object>> itemlist =  findInboundItemByShipmentId(shipmentId);
				...
				List<ShipmentItemDTO> dtoitemlist=new ArrayList<ShipmentItemDTO>();
				for (int i = 0; i < itemlist.size(); i++) {
					Map<String, Object> tempmap = itemlist.get(i);
					Integer quantity = 0;
					...
					ShipmentItemDTO dtoitem=new ShipmentItemDTO();
			...
			ShipmentDestinationAddressDTO dtoaddress=new ShipmentDestinationAddressDTO();
	...
	@Override
	public List<ShipInboundDestinationAddress> getDestination(String country) {
		return this.shipInboundDestinationAddressMapper.selectList(new LambdaQueryWrapper<ShipInboundDestinationAddress>().eq(ShipInboundDestinationAddress::getCountryCode,country));
	}

	@Override
	public void saveDestination(ShipInboundDestinationAddress destination) {
		ShipInboundDestinationAddress old = this.shipInboundDestinationAddressMapper.selectById(destination.getCode());
		if(old!=null){
			this.shipInboundDestinationAddressMapper.updateById(destination);
		}else{
			this.shipInboundDestinationAddressMapper.insert(destination);
		}
	}
}
