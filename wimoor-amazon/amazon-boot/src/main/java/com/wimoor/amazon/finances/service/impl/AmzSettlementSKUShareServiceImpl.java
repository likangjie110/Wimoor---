...
//	   Map<String,BigDecimal> feeMap=new ConcurrentHashMap<String,BigDecimal>();
//	   public BigDecimal getFee(String  key) {
...
//		   }
//	   }
//	   public void setFee(String  key,BigDecimal value) {
//		     feeMap.put(key,new BigDecimal(value.toString()));
//	   }
//	   public void addFee(String  key,BigDecimal value) {
//		   if(feeMap.get(key)!=null) {
//			   BigDecimal old = feeMap.get(key);
//			   old=old.add(value);
//			   feeMap.put(key, old); 
//		   }else {
	    	...
	    	if(transaction_type.equals("sku")) {
	    		...
	    		shopfee.shop_sales= item.get("sales")!=null?  Integer.parseInt(item.get("sales").toString()):shopfee.shop_sales;
	    			...
	    			if("Storage Fee".equals(amount_description)) {
	    				...
	    				//shopfee.addFee("shop_storage_fee", amount);
					}else if("FBA Long-Term Storage".equals(amount_description)||"StorageRenewalBilling".equals(amount_description)) {
						shopfee.shop_long_storage_fee=shopfee.shop_long_storage_fee.add(amount);
						//shopfee.addFee("shop_long_storage_fee", amount);
				    	...
				    	//shopfee.addFee("shop_coupon_redemption_fee", amount);
				    }else if("FBA Inventory Reimbursement".equals(amount_type)||"Manual Processing Fee Reimbursement".equals(amount_type)||"Item Fee Adjustment".equals(amount_type)){
				    	shopfee.shop_reimbursement_fee=shopfee.shop_reimbursement_fee.add(amount);
				    	//shopfee.addFee("shop_reimbursement_fee", amount);
				    }else if("DisposalComplete".equals(amount_description)||"RemovalComplete".equals(amount_description)){
				    	shopfee.shop_disposal_complete_fee=shopfee.shop_disposal_complete_fee.add(amount);
				    	//shopfee.addFee("shop_disposal_complete_fee", amount);
				    }else  {
				    	shopfee.shop_othershare=shopfee.shop_othershare.add(amount);
				    	//shopfee.addFee("shop_othershare", amount);
				   }
	    	}
	    }
...
//	  if(shopfeesub.getFee(type)==null) {  
//		  return new BigDecimal("0"); 
//		  }
//	  if(shopfeesub.getFee(type).compareTo(new BigDecimal("0"))==0) {
//		  return new BigDecimal("0");
//	  } else if(shopfeesub.getFee(type).floatValue()>0) {
//		  if(shopfeesub.getFee(type).floatValue()-value.floatValue()>0) {
//			  shopfeesub.setFee(type,shopfeesub.getFee(type).subtract(value));
//			  return value;
//		  }else {
//			  value= shopfeesub.getFee(type);
//			  shopfeesub.setFee(type,new BigDecimal("0"));
//			  return value;
//		  }
//	  }else {
//		  if(shopfeesub.getFee(type).floatValue()-value.floatValue()<0) {
...
//	  }
	  ...
	  }else if(type.equals("itemdisposal_fee")) {
		  if(shopfeesub.shop_disposal_complete_fee.compareTo(new BigDecimal("0"))==0) {
			  return new BigDecimal("0");
		  } else if(shopfeesub.shop_disposal_complete_fee.floatValue()>0) {
			  if(shopfeesub.shop_disposal_complete_fee.floatValue()-value.floatValue()>0) {
				  shopfeesub.shop_disposal_complete_fee=shopfeesub.shop_disposal_complete_fee.subtract(value);
				  return value;
			  }else {
				  value= shopfeesub.shop_disposal_complete_fee;
				  shopfeesub.shop_disposal_complete_fee=new BigDecimal("0");
				  return value;
			  }
		  }else {
			  if(shopfeesub.shop_disposal_complete_fee.floatValue()-value.floatValue()<0) {
...
//  	    BigDecimal itemStoragefee = shopfee.getFee("shop_storage_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//		BigDecimal itemlongStoragefee = shopfee.getFee("shop_long_storage_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemadvspend = shopfee.getFee("shop_advspend_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemcoupon_redemption_fee =shopfee.getFee("shop_coupon_redemption_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemreserve_amount =shopfee.getFee("shop_reserve_amount").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemreimbursement_fee=shopfee.getFee("shop_reimbursement_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemshopdisposal = shopfee.getFee("shop_disposal_complete_fee").multiply(rate).setScale(2, RoundingMode.HALF_UP);
//	    BigDecimal itemshopother = shopfee.getFee("shop_othershare").multiply(rate).setScale(2, RoundingMode.HALF_UP);
	    ...
	    BigDecimal itemshopdisposal = shopfee.shop_disposal_complete_fee.multiply(rate).setScale(2, RoundingMode.HALF_UP);
...
//    	item.setShareLongStorageFee(setValue(shopfeesub,itemlongStoragefee,"shop_long_storage_fee"));
//    	item.setShareAdvSpendFee(setValue(shopfeesub,itemadvspend,"shop_advspend_fee"));
//    	item.setShareCouponRedemptionFee(setValue(shopfeesub,itemcoupon_redemption_fee,"shop_coupon_redemption_fee"));
//    	item.setShareReserveFee( setValue(shopfeesub,itemreserve_amount,"shop_reserve_amount"));
//    	item.setShareReimbursementFee( setValue(shopfeesub,itemreimbursement_fee,"shop_reimbursement_fee"));
//    	item.setShareDisposalFee(setValue(shopfeesub,itemshopdisposal,"shop_disposal_complete_fee"));
//    	item.setShareShopOtherFee( setValue(shopfeesub,itemshopother,"shop_othershare"));
    	...
    	item.setShareDisposalFee(setValue(shopfeesub,itemshopdisposal,"itemdisposal_fee"));
  ...
  private void addLocalFeeToSku(AmzSettlementSummarySku item,Map<String,Object> localitem) {
			  /*</span>
		        ...
		        */
				...
				ProfitConfig config=profitCfgService.findConfigAction(configid);
				...
				if(mycountryconfig!=null) {
					  if(mycountryconfig.getVatRate()!=null) {
						  vat = setincome.multiply(mycountryconfig.getVatRate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  if(mycountryconfig.getLostrate()!=null) {
						  exchangelost = setincome.multiply(mycountryconfig.getLostrate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  ...
					  if(mycountryconfig.getCostrate()!=null) {
						 costrate = setincome.multiply(mycountryconfig.getCostrate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  if(mycountryconfig.getSellerrate()!=null
							  &&mycountryconfig.getSellerrate().compareTo(new BigDecimal("1000"))<0
							  &&mycountryconfig.getSellerrate().compareTo(new BigDecimal("0"))>0) {
						  marketfee = setincome.multiply(mycountryconfig.getSellerrate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  if(mycountryconfig.getCorporateInRate()!=null) {
						  companytax = setincome.multiply(mycountryconfig.getCorporateInRate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  if(mycountryconfig.getTaxrate()!=null) {
						  customstax = setincome.multiply(mycountryconfig.getTaxrate()).divide(new BigDecimal("100"),2,RoundingMode.HALF_UP);
					  }
					  if(config!=null){
				...
				if(marketfee.compareTo(BigDecimal.ZERO)<=0) {
					item.setProfitMarketfee(new BigDecimal(0)); 
				}else {
...
//		shopfeesub.setFee("shop_storage_fee", shopfee.getFee("shop_storage_fee"));
//		shopfeesub.setFee("shop_long_storage_fee", shopfee.getFee("shop_long_storage_fee"));
//		shopfeesub.setFee("shop_advspend_fee", shopfee.getFee("shop_advspend_fee"));
//		shopfeesub.setFee("shop_coupon_redemption_fee", shopfee.getFee("shop_coupon_redemption_fee"));
//		shopfeesub.setFee("shop_reserve_amount", shopfee.getFee("shop_reserve_amount"));
//		shopfeesub.setFee("shop_reimbursement_fee", shopfee.getFee("shop_reimbursement_fee"));
//		shopfeesub.setFee("shop_disposal_complete_fee", shopfee.getFee("shop_disposal_complete_fee"));
//		shopfeesub.setFee("shop_othershare", shopfee.getFee("shop_othershare"));
		...
		shopfeesub.shop_disposal_complete_fee=shopfee.shop_disposal_complete_fee;
...
//	    if(maxitem!=null&&(shopfeesub.getFee("shop_storage_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_long_storage_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_advspend_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_coupon_redemption_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_reserve_amount").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_reimbursement_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_disposal_complete_fee").compareTo(new BigDecimal("0"))!=0
//		||shopfeesub.getFee("shop_othershare").compareTo(new BigDecimal("0"))!=0
//		)) {
//	maxitem.setShareStorageFee(shopfeesub.getFee("shop_storage_fee").add(maxitem.getShareStorageFee()));
//	    maxitem.setShareLongStorageFee(shopfeesub.getFee("shop_long_storage_fee").add(maxitem.getShareLongStorageFee()));
//	    maxitem.setShareAdvSpendFee(shopfeesub.getFee("shop_advspend_fee").add(maxitem.getShareAdvSpendFee()));
//	    maxitem.setShareCouponRedemptionFee(shopfeesub.getFee("shop_coupon_redemption_fee").add(maxitem.getShareCouponRedemptionFee()));
//	    maxitem.setShareReimbursementFee(shopfeesub.getFee("shop_reimbursement_fee").add(maxitem.getShareReimbursementFee()));
//	    maxitem.setShareReserveFee( shopfeesub.getFee("shop_reserve_amount").add(maxitem.getShareReserveFee()));
//	    maxitem.setShareShopOtherFee( shopfeesub.getFee("shop_othershare").add(maxitem.getShareShopOtherFee()));
//	    maxitem.setShareDisposalFee(shopfeesub.getFee("shop_disposal_complete_fee").add(maxitem.getShareDisposalFee()));
//  }
	    if(maxitem!=null&&(shopfeesub.shop_storage_fee.compareTo(new BigDecimal("0"))!=0
	    	    ...
	    	    ||shopfeesub.shop_disposal_complete_fee.compareTo(new BigDecimal("0"))!=0
	    	...
	    	maxitem.setShareStorageFee(shopfeesub.shop_storage_fee.add(maxitem.getShareStorageFee()));
	 	    ...
	 	    maxitem.setShareDisposalFee(shopfeesub.shop_disposal_complete_fee.add(maxitem.getShareDisposalFee()));
	    }
	    if(skulist!=null&&skulist.size()>0) {
			amzSettlementSummarySkuMapper.delete(new LambdaQueryWrapper<AmzSettlementSummarySku>().eq(AmzSettlementSummarySku::getSettlementId, settlementid));
