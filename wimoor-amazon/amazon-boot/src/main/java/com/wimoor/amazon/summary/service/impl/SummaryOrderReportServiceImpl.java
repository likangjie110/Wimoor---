...
public class SummaryOrderReportServiceImpl implements ISummaryOrderReportService{
	...
	public Runnable productDateUpdateThread(final AmazonAuthority auth,final List<Map<String, Object>> list) {
		return new Runnable() {
			public void run() {
				try {
	...
	public void productDateUpdate(AmazonAuthority auth, List<Map<String, Object>> list) {
		...
		for (Map<String, Object> map : list) {
			if(map.get("msku")==null) {
				continue;
			}
			if(shopid==null) {
				shopid=list.get(0).get("shopid").toString();
			}
			mskulist.add(map.get("msku").toString());
		}
		Map<String, Map<String,Object>> data = erpClientOneFeign.getMaterialBySKUAction(shopid,mskulist);
		Map<String,Map<String, Object>> returnorderMap=new HashMap<String,Map<String, Object>>();
		List<Map<String, Object>> returnlist=amzOrderReturnsMapper.getBySellerId(auth.getSellerid());
		for (Map<String, Object> map : returnlist) {
			Object sku=map.get("sku");
			Object sales_channel=map.get("sales_channel");
			if(sku!=null&&sales_channel!=null){
				returnorderMap.put(sku.toString()+sales_channel.toString(),map);
			}
		}
		for (Map<String, Object> map : list) {
			// 更新销量信息表
			...
			String msku=map.get("msku").toString();
			if(msku.equals("KQ307")) {
				System.out.println("KQ307");
			}
			ProductInOrder productInOrder = new ProductInOrder();
			productInOrder.setPid(new BigInteger(map.get("pid").toString()));
			BigDecimal	price = calculate_Units(map,returnorderMap, productInOrder);// 七日平均售价
			...
			if(data!=null&&data.get(msku)!=null) {
				 materialMap =(Map<String, Object>) data.get(msku);
			}
			getProfit(productInProfit, price, map,buyprice,materialMap);
	...
	public BigDecimal calculate_Units(Map<String, Object> map,Map<String,Map<String, Object>> returnorderMap, ProductInOrder productInOrder) {
		...
		Marketplace market = this.marketplaceService.findMapByMarketplaceId().get(marketplaceid);
		if(returnorderMap!=null&&market!=null){
			Map<String,Object> returnMap = returnorderMap.get(sku + market.getPointName());
			if(returnMap!=null){
				if(returnMap.get("qty")!=null){
					Integer qty=Integer.parseInt(returnMap.get("qty").toString());
					Integer num=Integer.parseInt(returnMap.get("num").toString());
					productInOrder.setSalesReturnMonth(qty);
					productInOrder.setOrderReturnMonth(num);
				}
			}
		}
	...
	void getProfit(ProductInProfit sys, BigDecimal price, Map<String, Object> map, BigDecimal buyprice,Map<String, Object> materialMap) throws BizException {
		if (price == null || price.floatValue() == 0.0) {
			return;
		}
		String msku=map.get("msku").toString();
		...
		if(inputDimension_amz != null && inputDimension_amz.getLength() != null&& inputDimension_amz.getLength().getValue() != null) {
			String lengthValue = inputDimension_amz.getLength().value.toString() +
					(inputDimension_amz.getLength().units!=null?inputDimension_amz.getLength().units:"cm");
			String widthValue = inputDimension_amz.getWidth().value.toString() +
					(inputDimension_amz.getWidth().units!=null?inputDimension_amz.getWidth().units:"cm");
			String heightValue = inputDimension_amz.getHeight().value.toString() +
					(inputDimension_amz.getHeight().units!=null?inputDimension_amz.getHeight().units:"cm");
			String weightValue = inputDimension_amz.getWeight().value.toString() +
					(inputDimension_amz.getWeight().units!=null?inputDimension_amz.getWeight().units:"kg");
