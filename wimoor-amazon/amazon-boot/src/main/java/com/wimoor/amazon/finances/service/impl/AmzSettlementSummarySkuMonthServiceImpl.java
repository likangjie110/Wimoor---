...
final AmzOrderRemovesMapper amzOrderRemovesMapper;
	...
	private Map<String, BigDecimal> findSku_Reimbursements(Map<String, Object> param) {
 	...
 	private Map<String,BigDecimal> findSku_FirstShipment(Map<String, Object> map) {
 		Map<String,BigDecimal> result=new HashMap<String,BigDecimal>();
		 try{
			 Integer num = shipInboundTransMapper.needShareFee(map);
			 if(num!=null&&num>0){
 	...
 	private Map<String,Map<String, Object>> findSku_Disposal(Map<String, Object> map) {
		List<Map<String, Object>> list = amzOrderRemovesMapper.findSku_Disposal(map);
		Map<String,Map<String, Object>> result=new HashMap<String,Map<String, Object>>();
		for(Map<String, Object> item:list) {
			if(item.get("sku")!=null) {
				result.put(item.get("sku").toString(),item);
 	...
 	private Map<String,Map<String,Object>> findSku_InOutShipment(Map<String, Object> map) {
		...
		Map<String,Map<String,Object>> result=new HashMap<String,Map<String,Object>>();
 		for(Map<String, Object> item:list) {
 			if(item.get("sku")!=null) {
 				result.put(item.get("sku").toString().trim(),item);
 			}
 		}
 		return  result;
    ...
    private AmzSettlementSummarySkuMonth currencyExchange(
    	...
    	one.setShareDisposalFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getShareDisposalFee(), 2));
    	...
    	one.setRptDisposalFee(exchangeRateHandlerService.changeCurrencyByLocal(shopid, fromcurrency, tocurrency, item.getRptDisposalFee(), 2));
    	one.setRptDisposalUnits(item.getRptDisposalUnits());
    	...
    	one.setLocalUnitPrice(exchangeRateHandlerService.changeCurrencyByLocal(shopid,"CNY", tocurrency,item.getLocalUnitPrice(), 2));
        ...
        //利润：结算收入-SP广告费用-采购成本-运费预估-其他费用
 		...
 		env.put("share_disposalfee",getValue( one.getShareDisposalFee()));
		...
		env.put("disposalunits", one.getRptDisposalUnits()!=null?one.getRptDisposalUnits():0);
		env.put("disposalFee", getValue(one.getRptDisposalFee()));
 		...
 		env.put("unitprice", getValue(one.getLocalUnitPrice()));
    	...
    	env.put("profit_lostrate", getValue(one.getProfitLostrate()));
	...
	public void summaryMonthly(String amazonauthid, String marketplaceName, Date settlementStartDate) {
		// TODO Auto-generated method stub
		Integer repeatSummaryNumber=0;
		if(marketplaceName.contains("#")){
			repeatSummaryNumber=Integer.parseInt(marketplaceName.split("#")[1]);
			marketplaceName=marketplaceName.split("#")[0];
		}
		...
		param.put("currency",market.getCurrency());
		param.put("shopid",auth.getShopId());
		param.put("groupid",auth.getId());
		...
		Map<String,Map<String, Object>> disposalFee=findSku_Disposal(param); 
		...
		for(AmzSettlementSummarySkuMonth item:list) {
			...
			setExtendsData(item,storagefee,longfee,firstShipmentfee,advfee,reimbursementsFee,inoutShipment,disposalFee);
			...
			totalprice=totalprice.add(countrycurrency.getPrincipal());
		...
		BigDecimal totalPrincipal = this.baseMapper.summaryPrincipal(amazonauthid, marketplaceName, startdate);
//		if(checkDifferent(totalprice,totalPrincipal)&&repeatSummaryNumber<3){
//			summaryMonthly(amazonauthid,marketplaceName+"#"+repeatSummaryNumber,settlementStartDate);
//		}
		finDataFee.clear();
		advfee.clear();
	...
	private boolean checkDifferent(BigDecimal totalprice, BigDecimal totalPrincipal) {
		if(totalprice==null&&totalPrincipal==null){
			return false;
		}
		if(totalprice==null&&totalPrincipal!=null){
			return true;
		...
		return totalprice.intValue()!=totalPrincipal.intValue();
	}
	...
	private void setExtendsData(AmzSettlementSummarySkuMonth item, Map<String, BigDecimal> storagefee,
			...
			Map<String, Map<String, Object>> inoutShipment,Map<String, Map<String, Object>> disposalFee) {
		// TODO Auto-generated method stub
		...
		Map<String, Object> disposalFeeitem = disposalFee.get(sku);
		if(disposalFeeitem!=null) {
			item.setRptDisposalFee(disposalFeeitem.get("disposal")!=null?new BigDecimal(disposalFeeitem.get("disposal").toString()):new BigDecimal("0"));
			item.setRptDisposalUnits(disposalFeeitem.get("units")!=null?  Integer.parseInt(disposalFeeitem.get("units").toString()): Integer.parseInt("0"));
		}else {
			item.setRptDisposalFee(new BigDecimal("0"));
			item.setRptDisposalUnits(0);
		}
	...
	public void calcuteRefundRate(Map<String, Object> item){
	   if(item.get("order_amount")!=null&&item.get("refundorder")!=null) {
			BigDecimal order_amount = new BigDecimal(item.get("order_amount").toString());
			BigDecimal refundorder = new BigDecimal(item.get("refundorder").toString());
			if(order_amount.floatValue()>=0.001) {
				BigDecimal refundrate = refundorder.multiply(new BigDecimal("100")).divide(order_amount,2,RoundingMode.HALF_UP);
