...
public class ProfitServiceImpl implements IProfitService{
	...
	public static String[] smlAndLightCountry={"US","CA","UK","DE","FR","IT","SE","NL","PL","ES","JP","AU"};
	...
	public List<Map<String, String>> calculateSellingPrice(CostDetail costDetail, int typeId, String country, BigDecimal referralrate) {
		...
		if("UK".equals(country)){//2020年09月01日起，在UK站点加收2%的数字服务税。
			...
			minimum = minimum.multiply(new BigDecimal("0.25"));
		}
	...
	public Map<String, BigDecimal> estimateSellingPrice(CostDetail costDetail,BigDecimal referralR,BigDecimal minimum,BigDecimal FBA,
		...
		BigDecimal sellingPrice = m.divide(d, 2, RoundingMode.HALF_UP);
...
//		currencyUnitList.add("€");
	...
	public BigDecimal determineOutboundWeight(String country, ProductTier productTier, InputDimensions inputDimension, String isMedia) throws BizException {
		...
		outboundWeight = weight.add(boxweight).multiply(new BigDecimal("1000")).setScale(2, RoundingMode.CEILING);// 进位取整数，模拟不足一克按一克算,单位：g
		outboundWeight = outboundWeight.divide(new BigDecimal("1000"),3,RoundingMode.CEILING);
	...
	public InputDimensions initDimensionSort(InputDimensions inputDimension) {
		if (inputDimension != null) {
			...
			list.add(inputDimension.getLength().getValue()!=null?inputDimension.getLength().getValue():BigDecimal.ZERO);
			list.add(inputDimension.getWidth().getValue() !=null?inputDimension.getWidth().getValue():BigDecimal.ZERO);
			list.add(inputDimension.getHeight().getValue()!=null?inputDimension.getHeight().getValue():BigDecimal.ZERO);
	...
	public Map<String, String> getProductTier(ProfitConfigCountry profitConfigCountry, InputDimensions inputDimension, String country, String isMedia,
			BigDecimal price, boolean isSmlAndLight, String type,Boolean sipp) throws BizException {
		...
		if (profitServiceX != null) {
			...
			if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(country)) {
				...
				if (productTierId != null) {
					...
					FBA = profitServiceX.calculateSmlFBA(country, productTierId, outboundWeight,profitConfigCountry,type);//各个国家重写的方法
					BigDecimal sippfee=profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia, type,outboundWeight, profitConfigCountry,sipp);
					FBA=FBA.subtract(sippfee);
					...
					if (productTierId != null) {
						...
						FBA = profitServiceX.calculateFBA(country, productTierId, inputDimensionnew,isMedia, type, outboundWeight, profitConfigCountry, "national");
						BigDecimal sippfee =profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia, type,outboundWeight, profitConfigCountry,sipp);
						FBA=FBA.subtract(sippfee);
					}
				}
	...
	public CostDetail initCostDetail(String country, ProfitConfig profitCfgAll, InputDimensions inputDimension,
			...
			BigDecimal taxrate, BigDecimal gstrate, BigDecimal sellingGSTRate,BigDecimal referralrate, boolean isSmlAndLight,Boolean sipp) 
					throws BizException {
		// 对用户输入的长宽高重新排序
		...
		if (channel.contains("fba")) {// 如果利润计算方案是FBA，印度目前只支持FBA
			if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(country)) {
				String productTierId = profitServiceX.determineSmlProductTier(country, inputDimension,isMedia);// 公用方法
				if (productTierId == null) {
					return null;
				}
				...
				BigDecimal FBA = profitServiceX.calculateSmlFBA(country, productTierId, outboundWeight,profitConfigX,type);
				
				BigDecimal sippfee =new BigDecimal("0");
				if(sipp!=null&&sipp==false) {
					sippfee=profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia, type,outboundWeight, profitConfigX,sipp);
				}
				FBA=FBA.subtract(sippfee);
				...
				BigDecimal outboundWeight = profitServiceX.determineOutboundWeight(country, productTier, inputDimension,isMedia);
				costDetail.setProductTier(productTierName);
				costDetail.setOutboundWeight(outboundWeight);
				...
				BigDecimal FBA = profitServiceX.calculateFBA(country, productTierId, inputDimension, isMedia, type,outboundWeight, profitConfigX, shipmentType);
				BigDecimal sippfee =profitServiceX.calculateFBASIPP(country, productTierId, inputDimension, isMedia, type,outboundWeight, profitConfigX,sipp);
				FBA=FBA.subtract(sippfee);
				costDetail.setFBA(FBA);
				costDetail.setSipp(sippfee);
			}
			...
			if ("CA".equals(country)) {
				BigDecimal fbaTaxFee = costDetail.getFBA().multiply(fbaGSTTax).setScale(4, RoundingMode.HALF_UP);
				costDetail.setFbaTaxFee(fbaTaxFee);
			}
		...
		if (referralrate!=null) {
			referralrate = referralrate.divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		}
		
		//Currency Transport fee 計算方式：销售额 减去 亚马逊直接的收费之后(FBA费, 拥金, storage费等) , 余额部份再 X 汇率损耗率
		BigDecimal lostrate = profitConfigX.getLostrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//Currency Transport fee
		BigDecimal sellerrate = profitConfigX.getSellerrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//Marketing
		BigDecimal costrate = profitConfigX.getCostrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//其它每单销售固定成本比率
//		BigDecimal promotion = profitConfigX.getPromotion().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//all store promotion
		BigDecimal rateFee = sellerrate.add(costrate);//所有售价*rateFee的费用
		BigDecimal others = profitConfigX.getOtherfee();//OTHERS
		//VAT增值税费率
		BigDecimal vatRate=profitConfigX.getVatRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		...
		//进口GST税率
		gstrate = gstrate.divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//销售GST税率
		sellingGSTRate=sellingGSTRate.divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//企业所得税率
		BigDecimal corporateInRate=profitConfigX.getCorporateInRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
...
//		costDetail.setPromotionRate(promotion);
		...
		//进口关税=申报价值 * 关税税率
		BigDecimal tax = declaredValue.multiply(taxrate).setScale(4, RoundingMode.HALF_UP);
		costDetail.setTax(tax);
		//进口GST税费=(申报价值+关税) * GST税率
		BigDecimal import_gst=(new BigDecimal("1").add(taxrate)).multiply(declaredValue)
				.multiply(gstrate).setScale(4, RoundingMode.HALF_UP);
	...
	BigDecimal calculateShipment(ProfitConfig profitcfg,ProfitConfigCountry profitConfigX,String currency, String to, InputDimensions inputDimension,BigDecimal shipment){
		...
		if(shipment!=null &&shipment.floatValue()>0.0001) {//profitcfg.getShipmentstyle().equals("manually")如果手动输入了运费，shipment用户页面输入的运费 
			...
			BigDecimal dimensionalWeight = volum.divide(constantd,8, RoundingMode.HALF_UP);
			...
			shipment = greater.multiply(profitConfigX.getConstantw());
			shipment = this.exchangeRateHandlerService.changeCurrencyByLocal("RMB", to, shipment);
			...
			BigDecimal constantd = profitConfigX.getConstantd();
			BigDecimal dimensionalWeight = volum.divide(constantd,4, RoundingMode.HALF_UP);
			if(dimensionalWeight==null) {
				return null;
			}
			shipment = dimensionalWeight.multiply(profitConfigX.getConstantw());
			shipment = this.exchangeRateHandlerService.changeCurrencyByLocal("RMB", to, shipment);
		}
		return shipment;
	...
	public CostDetail getProfitByLocalData(String country, ProfitConfig profitcfg, InputDimensions inputDimension_amz, InputDimensions inputDimension_local,
		...
		if (channel.contains("fba")) {// 如果利润计算方案是FBA，印度目前只支持FBA
			Map<String, String> result = this.getProductTier(profitConfigX, inputDimension_amz, country, isMedia, price, isSmlAndLight,type,null);
			...
			// 亚马逊收费GST 税率
			BigDecimal fbaGSTTax = profitConfigX.getFbaTaxes().divide(new BigDecimal("100"), 4, RoundingMode.HALF_UP);
			costDetail.setFbaTax(fbaGSTTax);
			if ("CA".equals(country)) {
				BigDecimal fbaTaxFee = costDetail.getFBA().multiply(fbaGSTTax).setScale(4, RoundingMode.HALF_UP);
				costDetail.setFbaTaxFee(fbaTaxFee);
			...
			if ("IN".equals(country)) {
				...
				costDetail.setClosingFee(closingFee);
				//亚马逊收费GST税率默认18%, 应用于亚马逊销售佣金, Fixed Closing Fee, FBA fee等
				BigDecimal fbaTaxFee=costDetail.getFBA().add(closingFee).add(referralFee).multiply(fbaGSTTax).setScale(4, RoundingMode.HALF_UP);
				costDetail.setFbaTaxFee(fbaTaxFee);
			}
...
//		BigDecimal promotion = profitConfigX.getPromotion().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);//all store promotion
		...
		BigDecimal vatRate=profitConfigX.getVatRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//进口关税税率
		BigDecimal taxrate = profitConfigX.getTaxrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//进口GST税率
		BigDecimal gstrate=profitConfigX.getGstrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//销售GST税率
		BigDecimal sellingGSTRate=profitConfigX.getSellingGSTRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//企业所得税率
		BigDecimal corporateInRate=profitConfigX.getCorporateInRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
...
//		costDetail.setPromotionRate(promotion);
		...
		BigDecimal vatFee = price.multiply(costDetail.getVatRate()
				.divide(costDetail.getVatRate().add(new BigDecimal("1")), 4, RoundingMode.HALF_UP));
		// 销售GST税费=含税售价*gst/(1+gst)
		BigDecimal selling_GST = price.multiply(costDetail.getSelling_GSTRate()
				.divide(costDetail.getSelling_GSTRate().add(new BigDecimal("1")), 4, RoundingMode.HALF_UP));
		...
		BigDecimal margin = profit.multiply(new BigDecimal("100")).divide(price, 2, RoundingMode.HALF_EVEN);
	...
	public CostDetail getProfitByAmazonData(String country, ProfitConfig profitCfgAll, InputDimensions inputDimension_local, String isMedia, 
		...
		if ("grams".equalsIgnoreCase(fbaFee.getUnitOfWeight())) {
			if(fbaFee.getItemPackageWeight()!=null){
				BigDecimal weight = fbaFee.getItemPackageWeight().divide(new BigDecimal("1000"), 4, RoundingMode.HALF_UP);
...
//		referralFeeService.calculateReferralFee(typeid, price, country);
		if(price.setScale(2,RoundingMode.HALF_EVEN).floatValue()==fbaFee.getYourPrice().floatValue()){
			referralFee = fbaFee.getEstimatedReferralFeePerUnit();
		} else {
			BigDecimal referralRate = fbaFee.getEstimatedReferralFeePerUnit().divide(fbaFee.getYourPrice(),2,RoundingMode.HALF_EVEN);//比率=佣金/售价；
			referralFee = price.multiply(referralRate).setScale(2,RoundingMode.HALF_EVEN);
		...
		if (channel.contains("fba")) {// 如果利润计算方案是FBA
			...
			ProductTier productTier = productTierService.selectByPKey(productTierId);
			String productTierName = productTier.getAmzName()==null?productTier.getName():productTier.getAmzName();
			costDetail.setProductTier(productTierName);
			costDetail.setProductTierId(productTierId); 
...
//			}
			...
			if ("IN".equals(country)) {
				...
				//亚马逊收费GST税率默认18%, 应用于亚马逊销售佣金, Fixed Closing Fee, FBA fee等
				BigDecimal fbaTaxFee=fba.add(referralFee).add(fixclosingFee).multiply(profitConfigX.getFbaTaxes()).divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
				costDetail.setFbaTaxFee(fbaTaxFee);
			}
			if ("CA".equals(country)) {
				BigDecimal fbaTaxFee = fba.multiply(profitConfigX.getFbaTaxes()).divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
				costDetail.setFbaTaxFee(fbaTaxFee);
			}
			...
			BigDecimal inPlaceFee = profitServiceX.calculateInPlaceFee(country, productTier, inputDimensionNew,profitConfigX.getInvplacefee());// 加复选框，可选择不加上这个成本；如果选中要计算，则按产品重量对应计上成本,只有US和JP有这个费用选项
			costDetail.setInPlaceFee(inPlaceFee);
		...
		BigDecimal GSTRate=profitConfigX.getGstrate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//销售GST税率
		BigDecimal selling_GSTRate=profitConfigX.getSellingGSTRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
		//企业所得税率
		BigDecimal corporateInRate=profitConfigX.getCorporateInRate().divide(new BigDecimal("100"),4,RoundingMode.HALF_UP);
...
//		汇率损耗费=[售价-FBA（所有亚马逊直接收费）-佣金]*lostrate;
		...
		currencyLostFee = currencyLostFee.floatValue()>0?currencyLostFee:new BigDecimal("0");
//		vat增值税费=售价*vat/(1+vat)
		BigDecimal vatFee=price.multiply(costDetail.getVatRate().divide(costDetail.getVatRate().add(new BigDecimal("1")),4,RoundingMode.HALF_UP));
//		销售GST税费=含税售价*gst/(1+gst)
		BigDecimal selling_GST=price.multiply(costDetail.getSelling_GSTRate().divide(costDetail.getSelling_GSTRate().add(new BigDecimal("1")),4,RoundingMode.HALF_UP));
//		企业所得税 = 税率 * (销售价 - 以上所有成本(除了进口GST税, FBA的GST税))
...
//		成本=purchase+shipMengt+others+FBA（所有亚马逊直接收费,含佣金）+比率*售价+汇率损耗费+vat+销售GST税+进口关税+进口GST税费;
	...
	//根据售价计算产品成本详情及利润
	public CostDetail getCostDetail(ProfitQuery query,CostDetail costDetail, int typeId, BigDecimal referralrate, boolean isSmlAndLight,ProfitConfig profitcfg,Boolean sipp ) {
		BigDecimal price = costDetail.getSellingPrice();
		ReferralFee ref = referralFeeService.getById(typeId);
		if (price != null && price.doubleValue()>0) {
			...
			if ("IN".equals(costDetail.getCountry())) {
				if(referralrate!=null){
					referralFee = price.multiply(referralrate.divide(new BigDecimal("100"),4,RoundingMode.HALF_UP));
				}
				IProfitService profitServiceX = null;
				try {
			...
			if(profitcfg!=null) {
				...
				if (isSmlAndLight && Arrays.asList(smlAndLightCountry).contains(costDetail.getCountry())) {
					...
					BigDecimal FBA = profitServiceX.calculateSmlFBA(costDetail.getCountry(), costDetail.getProductTierId(),
							costDetail.getOutboundWeight(),cfgcountry,ref.getType());
					FBA=FBA.subtract(costDetail.getSipp()!=null?costDetail.getSipp():new BigDecimal("0"));
					costDetail.setFBA(FBA);
				}
			}
			...
			BigDecimal profit = price.subtract(totalCost);
			BigDecimal margin = profit.multiply(new BigDecimal("100")).divide(price, 2, RoundingMode.HALF_UP);
	...
	public String determineSmlProductTier(String country, InputDimensions inputDimension,String isMedia ) {
		...
		InputDimensions dimensions = new InputDimensions(length, width, height, getDimUnit(country), weight,getWeightUnit(country));
		dimensions.setDimensionalWeight(dimensions.getDimensionalWeight(getDimUnit(country)));
		...
		for (int i = 0; i < tierList.size(); i++) {
			...
			if(format!=null&&format.contains("isMedia")) {
				  dimensions.getGirth(dimensions.getHeight().getUnits());
				  result = (Boolean) AviatorEvaluator.exec(format, dimensions,isMedia);
			}else {
				 result = (Boolean) AviatorEvaluator.exec(format, dimensions);
			}
		...
		if (productTier!=null&&boxweight.compareTo(new BigDecimal("0"))>0) {
			productTier=null;
			weight = weight.add(boxweight).setScale(2, RoundingMode.CEILING);
...
//	   this.referralMap.clear();
	...
	public BigDecimal calculateReferralFee(int typeId, BigDecimal sellingPrice, String country) {
		...
		if(country.equals("US")&&typeId==11) {
			if(sellingPrice.subtract(resultfee.getTop2()).doubleValue()>0) {
				referralFee = sellingPrice.multiply(resultfee.getPercent3());
				return referralFee;
			}else if(sellingPrice.subtract(resultfee.getTop1()).doubleValue()>0){
	...
	public BigDecimal calculateFBASIPP(String country, String productTierId, InputDimensions inputDimension,
		...
		if(sipp==null&&(profitConfigX==null||profitConfigX.getSipp()==false)) {
			return FBA;
		}
		FBASipp  sippfee = iFBASippService.findByProductTierIdNew(productTierId,isClothing,country,outboundWeight);
		if(sippfee==null||sippfee.getFormat()==null) {
			return new BigDecimal("0");
		}
		String format = sippfee.getFormat();
		if (format.contains("?")) {// 如果有逻辑判断
			format = (String) AviatorEvaluator.exec(format, outboundWeight);
		}
		if(format.contains("{0}")) {
			format = StringFormat.format(format, outboundWeight);
		}
		if(format.contains("value")) {
			format =  AviatorEvaluator.exec(format, outboundWeight).toString();
		}
		FBA =  new BigDecimal(Calculator.conversion(format));
		return FBA;
