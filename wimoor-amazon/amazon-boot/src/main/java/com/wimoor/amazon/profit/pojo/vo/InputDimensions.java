...
public 	class InputDimensions {
	...
	private BigDecimal dimBase=new BigDecimal("5000");
	...
	public BigDecimal getDimBase() {
		return dimBase;
	}
	public void setDimBase(BigDecimal dimBase) {
	...
	public ItemMeasure getDimensionalWeight(String toUnit) throws BizException {
		if (toUnit!=null&&toUnit.equals(unit_in)) {
			...
			BigDecimal d = getVolume(toUnit).getValue().divide(new BigDecimal("139"),4,RoundingMode.HALF_UP);
			...
			BigDecimal d = getVolume(toUnit).getValue().divide(dimBase,4,RoundingMode.HALF_UP);
			dimensionalWeight = new ItemMeasure(d, toUnit);
		}
		return dimensionalWeight;
	}
	...
	public ItemMeasure getDimensionalWeight(BigDecimal baseData) throws BizException {
		    BigDecimal d = getVolume(InputDimensions.unit_cm).getValue().divide(baseData,4,RoundingMode.HALF_UP);
	...
	public ItemMeasure getDimensionalWeight(String toUnit,BigDecimal length_in,BigDecimal width_in,BigDecimal height_in) {
		if (toUnit.equals(unit_in)) {
			...
			BigDecimal d = getVolume(toUnit).getValue().divide(new BigDecimal("5000"),4,RoundingMode.HALF_UP);
			dimensionalWeight = new ItemMeasure(d, toUnit);
		}
		return dimensionalWeight;
	}
	...
	public ItemMeasure ConversionTo(String toUnit, ItemMeasure measure) throws BizException {// 只支持cm和in转换
		...
		if("cm".equals(toUnit)) {
			return new ItemMeasure(value.setScale(1, RoundingMode.HALF_UP), toUnit);
		}else {
			return new ItemMeasure(value.setScale(2, RoundingMode.HALF_UP), toUnit);
		}
	}
