...
public class ShipInboundPlanServiceImpl extends ServiceImpl<ShipInboundPlanMapper,ShipInboundPlan> implements IShipInboundPlanService { 
	...
	@Autowired
	@Lazy
	IAmzInboundFbaCycleService iAmzInboundFbaCycleService;
	@Autowired
	@Lazy
	IFBAShipCycleService iFBAShipCycleService;
   ...
   private ShipInboundPlan createPlan(AmazonAuthority auth, Marketplace market, InboundShipmentInfo item,String warehouseid,Integer skunum) {
		...
		if(list!=null&&list.getTotal()>0) {
			...
			List<SummaryShipmentVo> listshipment = showPlanListByPlanid(inboundplanid,null);
			 for(SummaryShipmentVo item:listshipment) {
				 ...
				 AmzShipFulfillmentCenter center = iAmzShipFulfillmentCenterService.getByCode(item.getDestinationFulfillmentCenterId());
				 if(center!=null) {
					 item.setAddressarea(center.getArea());
					 item.setIsfar(center.getIsfar());
				 }else {
					 item.setAddressarea("地域未知");
				 }
	...
	public ChartLine shipArrivalTimeChart(String groupid,String amazonAuthId, String sku,String marketplaceid, int daysize,UserInfo userinfo) {
		...
		List<AmzInboundFbaCycle> cycleList = iAmzInboundFbaCycleService.getInboundFbaCycle(userinfo.getCompanyid() ,marketplaceid);
		AmzInboundFbaCycle myfbacycle =iAmzInboundFbaCycleService.getDefaultInboundFbaCycle(cycleList);
		FBAShipCycle cycle = iFBAShipCycleService.getFbaShipCycle(groupid,marketplaceid,sku);
		BigDecimal stocking_cycle =cycle!=null&&cycle.getStockingcycle()!=null? new BigDecimal(cycle.getStockingcycle()):new BigDecimal(0);
		if(cycle!=null&&cycle.getTranstype()!=null) {
			...
			stocking_cycle =myfbacycle!=null&&myfbacycle.getStockingCycle()!=null? new BigDecimal(myfbacycle.getStockingCycle()):new BigDecimal(0);
		}
