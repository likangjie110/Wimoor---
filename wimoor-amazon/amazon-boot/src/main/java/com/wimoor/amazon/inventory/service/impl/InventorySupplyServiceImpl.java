...
import com.amazon.spapi.model.fbainventory.ReservedQuantity;
...
public class InventorySupplyServiceImpl implements IInventorySupplyService{
	...
	public Map<String, InventorySummary> captureInventorySupplyNew(AmazonAuthority amazonAuthority, Date date) {
		...
		try {
			...
			while(nexttoken!=null) {
				     response = api.getInventorySummaries("Marketplace", market.getMarketplaceid(),Arrays.asList(market.getMarketplaceid()), null, null, null,null, nexttoken);
				     if(response!=null) {
	...
	public Map<String, InventorySummary> captureInventorySupplyNew(AmazonAuthority amazonAuthority, List<String> list) {
		...
		try {
			...
			while(nexttoken!=null) {
				     response = api.getInventorySummaries("Marketplace", market.getMarketplaceid(), null, null, null,null, null, nexttoken);
				 ...
				 if(invdetail!=null) {
					 ...
					 if(infolist!=null&&infolist.size()>0) {
					     ...
					     if(report==null) {
							 ...
							 if(amazonAuthority.getMarketPlace().getRegion().equals("EU")){
								 report.setMarketplaceid("EU");
							 }else{
						 ...
						 if(invdetail.getReservedQuantity()!=null) {
							 ...
							 LambdaQueryWrapper<InventoryReservedReport> query=new LambdaQueryWrapper<InventoryReservedReport>();
							 query.eq(InventoryReservedReport::getSku, resdetail.getSku());
							 query.eq(InventoryReservedReport::getAmazonAuthId, resdetail.getAmazonAuthId());
							 query.eq(InventoryReservedReport::getMarketplaceid, resdetail.getMarketplaceid());
							 InventoryReservedReport oldres = inventoryReservedReportMapper.selectOne(query);
							 if(oldres!=null) {
