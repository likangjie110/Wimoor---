...
public class ProductInOptServiceImpl extends ServiceImpl<ProductInOptMapper, ProductInOpt> implements IProductInOptService {
	...
	public List<Map<String, Object>> findMaterialSizeByCondition(Map<String, Object> param){
		...
		for (int i = 0; i < totalresult.size(); i++) {
			...
			Object efnfbauk = item.get("efnfbauk");// fbaFee.getExpectedEfnFulfilmentFeePerUnitUk();
			Object efnfbade = item.get("efnfbade");// fbaFee.getExpectedEfnFulfilmentFeePerUnitUk();
			if(total!=null&&referral!=null&&total.toString().equals(referral.toString())){
				if(efnfbauk!=null){
					total=new BigDecimal(referral.toString()).add(new BigDecimal(efnfbauk.toString()));
				}
				if(efnfbade!=null){
					total=new BigDecimal(referral.toString()).add(new BigDecimal(efnfbade.toString()));
				}
			}
			...
			if (total == null || referral == null) {
				countrytirMap = profitService.getProductTier
						(profitConfigCountry, countryInputDimension, country, "0", sellprice, isSmlAndLight, pgroup,null);
...
//						} else {
//							efnfbafee =  new BigDecimal(efnfbade.toString());
//						}
//					} else {
//						efnfbafee = new BigDecimal(efnfbauk.toString());
//					}
					amzFBA = fab_report;
	...
	@Override
	public void updateOptOwner(String owner, List<String> pidlist,String ftype) {
		if("autolocal".equals(ftype)){
			//修改商品负责人为产品负责人
			List<String> mskulist=new ArrayList<String>();
			Map<String,ProductInOpt> mskuopt=new HashMap<String,ProductInOpt>();
			for (String pid:pidlist){
				...
				if(opt!=null&&opt.getMsku()!=null){
					...
					opt=new ProductInOpt();
					opt.setPid(new BigInteger(pid));
					ProductInfo info = this.productInfoMapper.selectById(pid);
					msku=info.getSku();
					opt.setMsku(msku);
				}
				mskuopt.put(msku, opt);
				mskulist.add(msku);
			}
			Map<String, Object> maps = erpClientOneFeign.findMaterialMapBySkuV2(mskulist);
			if(maps!=null){
                 for(Map.Entry<String, ProductInOpt> entry:mskuopt.entrySet()){
					 String msku = entry.getKey();
					 Map<String,Object> mskuinfo= maps.get(msku)==null?null: (Map<String, Object>) maps.get(msku);
					 if(mskuinfo!=null){
						 String ownerid=mskuinfo.get("owner")==null?null:mskuinfo.get("owner").toString();
						 if(StrUtil.isNotEmpty(ownerid)){
							 ProductInOpt opt = entry.getValue();
							 ProductInOpt oldopt = this.baseMapper.selectById(opt.getPid());
							 if(oldopt!=null){
								 oldopt.setOwner(ownerid);
								 this.baseMapper.updateById(oldopt);
							 }else{
			...
			if(StrUtil.isNotEmpty(owner) && pidlist!=null && pidlist.size()>0) {
				for (int i = 0; i < pidlist.size(); i++) {
					String pid=pidlist.get(i);
					ProductInOpt inopt = this.baseMapper.selectById(pid);
					if(inopt!=null) {
						inopt.setOwner(owner);
						this.baseMapper.updateById(inopt);
					}
				}
			}
		}
	...
	@Override
	public void updateFeeRate(UserInfo user, Float fee) {
		if(fee!=null) {
			AmazonDeclareRate feeinfo = amazonDeclareRateMapper.selectById(user.getCompanyid());
			if(feeinfo!=null) {
				feeinfo.setRate(fee);
				amazonDeclareRateMapper.updateById(feeinfo);
			}else {
				feeinfo=new AmazonDeclareRate();
				feeinfo.setRate(fee);
				feeinfo.setShopid(user.getCompanyid());
				amazonDeclareRateMapper.insert(feeinfo);
			}
		}
	}
	@Override
	public Object selectFeeRate(UserInfo user) {
		AmazonDeclareRate feeinfo = amazonDeclareRateMapper.selectById(user.getCompanyid());
		if(feeinfo!=null) {
			return feeinfo.getRate();
		}else {
			return null;
		}
	}
	...
	@Override
	public void downloadFeeRateList(SXSSFWorkbook workbook,String shopid,String groupid, String marketplaceid, String authid) {
		List<Map<String, Object>> list = amazonDeclareRateMapper.findFeeRateList(shopid,groupid,marketplaceid,authid);
		...
		titlemap.put("货件ID", "shipmentid");
		titlemap.put("平台SKU", "sku");
		titlemap.put("单个售价", "price");
		titlemap.put("申报比例", "rate");
		titlemap.put("单个申报金额", "ratefee");
		titlemap.put("发货出库时间", "shipdate");
	...
	@Override
	public List<ProductInfo> uploadLabelExcel(Sheet sheet, UserInfo user, String amazonAuthid, String marketplaceid) {
		List<ProductInfo> productInfoList=new ArrayList<ProductInfo>();
		Row whrow = sheet.getRow(0);
		Cell gpnamecell = whrow.getCell(1);
		String groupMarketName = gpnamecell.getStringCellValue();
		if (GeneralUtil.isNotEmpty(groupMarketName)) {
			for (int i = 2; i <= sheet.getLastRowNum(); i++) {
				...
				ProductInfo item = list.get(0);
				Cell cell2 = skuRow.getCell(1);
				if(cell2!=null){
					cell2.setCellType(CellType.STRING);
					String nums = cell2.getStringCellValue();
					if(StrUtil.isBlank(nums)){
						item.setTotalOfferCount(1);
					}else{
						item.setTotalOfferCount(Integer.parseInt(nums));
					}
				}
