...
public Map<String, Object> getReturnOrder(SummaryMutilParameterQueryDTO parameter) {
	 ...
	 Marketplace market =null;
	  ...
	  List<String> labels = ChartPoint.getLabels(ChartPoint.Daily, beginDate, endDate);
	  List<Object> quantityList = ChartPoint.getChartData(ChartPoint.Daily, listMap, "return_date", "quantity", beginDate, endDate);
...
public Map<String, Object> selectByMutilParameter(SummaryMutilParameterQueryDTO parameter) {
	 ...
	 Marketplace market =null;
	  ...
	  Date endDate=GeneralUtil.getDatez(parameter.getEndDate());
	  List<String> labels = ChartPoint.getLabels(ChartPoint.Daily, beginDate, endDate);
	  List<Object> quantityList = ChartPoint.getChartData(ChartPoint.Daily, listMap, "purchase_date", "quantity", beginDate, endDate);
	  List<Object> orderNumberList = ChartPoint.getChartData(ChartPoint.Daily, listMap, "purchase_date", "ordernumber", beginDate, endDate);
	  Map<String,Object> result=new HashMap<String,Object>();
	  result.put("labels", labels);
	  result.put("quantityList", quantityList);
...
public ChartLine findOrderSummaryBySku(String groupid,String amazonAuthId, String sku, String parentAsin,String marketplaceid,Integer daysize ,String linetype, UserInfo user) {
	...
	if(StrUtil.isNotBlank(parentAsin)) {
		param.put("parentAsin", parentAsin);
		param.put("sku", null);
	}else {
		param.put("sku", sku);
		param.put("parentAsin", null);
	}
	...
	if (list != null && list.size() > 0) {
	 ...
	 String yesterday=sdf.format(today.getTime());
		...
		String mysku =StrUtil.isNotBlank(parentAsin)?parentAsin: sku;
