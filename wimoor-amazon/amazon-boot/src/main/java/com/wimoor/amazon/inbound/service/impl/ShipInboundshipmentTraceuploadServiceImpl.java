...
public class ShipInboundshipmentTraceuploadServiceImpl extends ServiceImpl<ShipInboundshipmentTraceuploadMapper, ShipInboundshipmentTraceupload> implements IShipInboundshipmentTraceuploadService {
	...
	final ShipInboundShipmentV2Mapper ShipInboundShipmentV2Mapper;
	@Override
	...
	public void uploadTraceuploadFile(UserInfo user, Row info) {
		...
		if(StrUtil.isNotEmpty(shipmentid) && StrUtil.isNotEmpty(transnumber)) {
			LambdaQueryWrapper<ShipInboundShipment> shipmentV2Query=new LambdaQueryWrapper<ShipInboundShipment>();
			shipmentV2Query.eq(ShipInboundShipment::getShipmentConfirmationId, shipmentid);
			ShipInboundShipment shipment = ShipInboundShipmentV2Mapper.selectOne(shipmentV2Query);
			if(shipment!=null){
				...
				if(boxlist!=null && boxlist.size()>0) {
					for (ShipInboundShipmentBox boxitem:boxlist) {
						boxitem.setTrackingId(transnumber);
						boxitem.setOperator(user.getId());
						boxitem.setOpttime(new Date());
						shipInboundShipmentBoxMapper.updateById(boxitem);
					}
				}
			}else{
