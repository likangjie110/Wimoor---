...
public class AmzCouponPerformanceReportServiceImpl extends ServiceImpl<AmzCouponPerformanceReportMapper, AmzCouponPerformanceReport> implements IAmzCouponPerformanceReportService {
   final AmzCouponAsinMapper amzCouponAsinMapper;
    ...
    public IPage<Map<String, Object>> findByCondition(PerformanceReportListDTO dto) {
        ...
        if(page!=null&&page.getRecords().size()>0){
            for(Map<String, Object> map:page.getRecords()){
                LambdaQueryWrapper<AmzCouponAsin> query=new LambdaQueryWrapper<AmzCouponAsin>();
                ...
                query.eq(AmzCouponAsin::getCouponid, couponid);
                List<AmzCouponAsin> list = amzCouponAsinMapper.selectList(query);
                List<Map<String,Object>> asindetail=new LinkedList<Map<String,Object>>();
                for(AmzCouponAsin asin:list){
