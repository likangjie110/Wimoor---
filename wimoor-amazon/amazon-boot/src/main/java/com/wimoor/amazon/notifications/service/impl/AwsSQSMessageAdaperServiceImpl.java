...
public class AwsSQSMessageAdaperServiceImpl implements IAwsSQSMessageAdaperService{
	...
	@Resource
	IAwsSQSMessageHandlerService productProductPriceService;
	@Resource
			...
			return true;
			      ...
			      try {
			    	  JSONObject notificationVersion = GeneralUtil.getJsonObject(body);
						 ...
						 }else if(notificationType.equals("ORDER_CHANGE")||notificationType.equals("ORDER_STATUS_CHANGE")) {
						 ...
						 }else if(notificationType.equals("PRICE_HEALTH")){
							 handlerMsg(productFollowHandlerService,notificationVersion.getJSONObject("Payload"));
						 }
			      }catch(Exception e) {
			    	  e.printStackTrace();
