...
public class ShipAddressServiceImpl extends  ServiceImpl<ShipAddressMapper,ShipAddress>  implements IShipAddressService {
	...
	public IPage<ShipAddress> findByCondition(Page<?> page,String shopid,String groupid,String marketplaceid,String disable,String search) {
		if(GeneralUtil.isEmpty(search))search=null;
		else search=search.trim()+"%";
		return this.baseMapper.findByCondition(page,shopid,groupid,marketplaceid,disable,search);
	}
