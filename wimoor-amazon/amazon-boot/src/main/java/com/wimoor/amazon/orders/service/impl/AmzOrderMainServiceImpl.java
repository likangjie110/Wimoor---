...
public class AmzOrderMainServiceImpl extends ServiceImpl<AmzOrderMainMapper,AmzOrderMain> implements IAmzOrderMainService,IAwsSQSMessageHandlerService {
	...
	public int saveAddress(AmzOrderBuyerShipAddress entity) {
		...
		if(oldone!=null) {
			...
			if(entity.getMunicipality()!=null&&entity.getMunicipality().length()>20) {
				entity.setMunicipality(entity.getMunicipality().substring(0, 20));
			}
    ...
    public void handlerOrderResponse(AmazonAuthority auth,AmzAuthApiTimelimit apilimit,GetOrdersResponse response) {
		...
		if(orders!=null && orders.size()>0) {
			for(int i=0;i<orders.size();i++) {
				...
				query.eq(AmzOrderMain::getAmazonauthid, auth.getId());
				query.eq(AmzOrderMain::getAmazonOrderId, orderid);
				AmzOrderMain oldorder = this.getOne(query);
				...
				if(oldorder!=null) {
                   LambdaQueryWrapper<AmzOrderMain> queryUpdate=new LambdaQueryWrapper<AmzOrderMain>();
                   queryUpdate.eq(AmzOrderMain::getAmazonOrderId, entity.getAmazonOrderId());
                   queryUpdate.eq(AmzOrderMain::getAmazonauthid, entity.getAmazonauthid());
				   this.update(entity,queryUpdate);					
				}else {
					save(entity);
				}
			}
			            ...
			            if(apilimit.apiNotRateLimit()) {
							if(StrUtil.isBlank(token)) {
								...
								api.getOrdersAsync(marketplaceIds, null,null, lastUpdatedAfter,
										 ...
										 null, null, 100, 
										 null, null, null,null,null,null,null,null,null,null,null, orderReponseHandler); 
							}else {
								...
								api.getOrdersAsync(marketplaceIds, null, null, null,
										 null, null, fulfillmentChannels, paymentMethods, 
										 null, null, 100, 
										 null, null, token,null,null,null,null,null,null,null,null, orderReponseHandler); 
							}
			            }
	...
	public boolean handlerMessage(JSONObject body) {
		// TODO Auto-generated method stub
		JSONObject OrderChangeNotification =null;
		if(body.containsKey("OrderStatusChangeNotification")){
			OrderChangeNotification=body.getJSONObject("OrderStatusChangeNotification");
		}else{
			OrderChangeNotification=body.getJSONObject("OrderChangeNotification");
		}
		String sellerId=OrderChangeNotification.getString("SellerId");
		String amazonOrderId=OrderChangeNotification.getString("AmazonOrderId");
		JSONObject summary=OrderChangeNotification.getJSONObject("Summary");
		...
		if(summary!=null){
			  marketplaceId=summary.getString("MarketplaceId");
			  purchaseDate=summary.getDate("PurchaseDate");
			  orderStatus=summary.getString("OrderStatus");
			  destinationPostalCode=summary.getString("DestinationPostalCode");
			  fulfillmentType=summary.getString("FulfillmentType");
		}else{
			marketplaceId=OrderChangeNotification.getString("MarketplaceId");
			purchaseDate=OrderChangeNotification.getDate("PurchaseDate");
			orderStatus=OrderChangeNotification.getString("OrderStatus");
			destinationPostalCode=OrderChangeNotification.getString("DestinationPostalCode");
			fulfillmentType=OrderChangeNotification.getString("FulfillmentChannel");
		}
		...
		if(oldorder!=null) {
			...
			order.setFulfillmentChannel(fulfillmentType);
		...
		JSONArray orderItems = OrderChangeNotification.getJSONArray("OrderItems");
		if(orderItems!=null) {
			for(int i=0;i<orderItems.size();i++) {
				JSONObject item = orderItems.getJSONObject(i);
				String orderItemId=item.getString("OrderItemId");
				String sellerSKU=item.getString("SellerSKU");
				Integer quantity=item.getIntValue("Quantity");
				String supplySourceId=item.getString("SupplySourceId");
