...
public class AmzFinAccountController {
   ...
   @ApiOperation(value = "更新未出账账期")
   @GetMapping("/refreshAmzFinByAuth")
   public Result<?> refreshAmzFinByAuth(String authid) {
	log.info("更新未出账账期------"+new Date());
	iAmzFinAccountService.runApi(iAmazonAuthorityService.getById(authid));
       return Result.judge(true);
   }
   ...
   public Result<?> sendWeekEmailAction(String shopid) {
	      new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					 if(StrUtil.isBlankIfStr(shopid)) {
