...
import com.wimoor.erp.purchase.pojo.dto.PurchaseFormEntryChangeDTO;
import com.wimoor.erp.purchase.pojo.entity.*;
import com.wimoor.erp.purchase.service.IPurchaseFormEntryChangeReceiveService;
import com.wimoor.erp.purchase.service.IPurchaseFormEntryChangeService;
...
public class PurchaseFormEntryChangeController {
	...
	public Result<Map<String, Object>> saveDataAction(@RequestParam(value="file",required=false) MultipartFile file, PurchaseFormEntryChangeDTO inWarehouseForm)   {
		...
		if(StrUtil.isNotEmpty(inWarehouseForm.getEntryid())) {
			...
			if(StrUtil.isBlank(inWarehouseForm.getWarehouseid())){
				PurchaseFormEntry entry = purchaseFormEntryService.getById(inWarehouseForm.getEntryid());
				PurchaseForm form = purchaseFormService.getById(entry.getFormid());
				inWarehouseForm.setWarehouseid(form.getWarehouseid());
				inWarehouseForm.setSupplierid(entry.getSupplier());
