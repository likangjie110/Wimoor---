...
public class InventoryFormAgentServiceImpl implements IInventoryFormAgentService {
	...
	@Override
	public void refreshInventoryTask() {
		List<Inventory> list=inventoryService.findNeedRefresh();
			...
			public void run() {
				try {
					...
					for(Inventory  item:list) {
						Integer qty=null;
						if(item.getStatus().equals("inbound_assembly")){
							  qty = assemblyFormService.refreshInbound(item.getShopid(), item.getWarehouseid(), item.getMaterialid());
						}else{
							  qty=iPurchaseFormReceiveService.refreshInbound(item.getWarehouseid(), item.getMaterialid());
						}
						if(qty==null){
							qty=0;
						}
						if(qty.intValue()==item.getQuantity().intValue()) {
							continue;
						}
						inventoryService.repairInventory(user, item.getWarehouseid(), item.getMaterialid(), item.getStatus(), qty);
					}
					Thread.sleep(60000);
				} catch (Exception e) {
