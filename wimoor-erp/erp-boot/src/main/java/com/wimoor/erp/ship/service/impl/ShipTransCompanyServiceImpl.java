...
//		}
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++,c.add(Calendar.SECOND,1)) {
			Row skuRow = sheet.getRow(i);
			Cell cell = skuRow.getCell(0);
			cell.setCellType(CellType.STRING);
			String companyname = cell.getStringCellValue();
	...
	private String handlerChannelType(UserInfo user, String channeltype) {
		...
		if(StrUtil.isEmpty(channeltype)||"".equals(channeltype)) {
			...
			LambdaQueryWrapper<ShipTransChannel> query=new LambdaQueryWrapper<ShipTransChannel>();
			query.and(wp->wp.isNull(ShipTransChannel::getShopid).or(op->op.eq(ShipTransChannel::getShopid,user.getCompanyid())));
			query.eq(ShipTransChannel::getName, channeltype);
			ShipTransChannel oldone = shipTransChannelService.getOne(query);
			if(oldone!=null) {
				if(oldone.getDisabled()==null|| oldone.getDisabled()){
					oldone.setDisabled(false);
