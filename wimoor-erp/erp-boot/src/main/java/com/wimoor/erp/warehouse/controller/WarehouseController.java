...
public class WarehouseController {
	...
	@ApiOperation("海外仓库国家列表")
	@GetMapping("/getOverseaCountry")
	public Result<List<String>> getOverseaCountry(){
		UserInfo userinfo = UserInfoContext.get();
		// 获取当前登录用户公司对应的所有海外仓库
		List<Warehouse> warehouseList = warehouseService.getOverseaWarehouse(userinfo.getCompanyid(), "oversea_usable", null, null);
		// 提取所有唯一的国家信息
		Set<String> countrySet = warehouseList.stream()
		        .map(Warehouse::getCountry)
		        .filter(StrUtil::isNotBlank)
		        .collect(Collectors.toSet());
		// 转换为列表返回
		List<String> countryList = new LinkedList<>(countrySet);
