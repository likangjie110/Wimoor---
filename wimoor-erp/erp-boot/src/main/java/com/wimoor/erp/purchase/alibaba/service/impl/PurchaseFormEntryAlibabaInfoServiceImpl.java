	...
	public JSONObject captureAlibabaOrder(UserInfo user, String alibabaAuthid, String alibabaOrderid, String purchaseEntryid) {
		...
		if(result != null) {
			...
			if (oldone != null) {
				...
				PurchaseFormEntryAlibabaInfoExt oldnewext = purchaseFormEntryAlibabaInfoExtMapper.selectById(newext.getEntryid());
				if(oldnewext!=null){
					newext.setEntryid(oldnewext.getEntryid());
					this.purchaseFormEntryAlibabaInfoExtMapper.updateById(newext);
				}else{
					this.purchaseFormEntryAlibabaInfoExtMapper.insert(newext);
				}
			}
