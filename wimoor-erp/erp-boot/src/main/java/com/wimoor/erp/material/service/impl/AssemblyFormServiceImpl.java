...
//			}
	...
	public Map<String,Object> getLastOneFormByMaterial(String id,String warehouseid) {
		if (id == null)
			return null;
		List<Map<String,Object>> list = this.baseMapper.findLastByMaterial(id.toString().trim(),warehouseid,"one");
		if (list != null  && list.size()>0) {
	...
	public List<Map<String,Object>> getLastThreeFormByMaterial(String id) {
		...
		List<Map<String,Object>> list = this.baseMapper.findLastByMaterial(id.toString(),null,"three");
		for(Map<String,Object> item:list) {
			if(item!=null&&item.get("auditstatus")!=null) {
				item.put("auditstatusName",AssemblyForm.getAuditstatusName(Integer.parseInt(item.get("auditstatus").toString())))	;
			}
