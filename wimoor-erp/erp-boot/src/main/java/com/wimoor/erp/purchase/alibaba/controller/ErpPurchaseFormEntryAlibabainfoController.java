		...
		if(result!=null&&result.get("orderInfo")!=null) {
			...
			if(supplier!=null) {
				for(int i =0 ;i<productItems.size();i++) {
					...
					if((productID!=null&&productID.equals(supplier.getProductcode()))) {
...
//			 }
			...
			ErpPurchaseAlibabaProductitems item = null;
			LambdaQueryWrapper<ErpPurchaseAlibabaProductitems> query=null;
			query=new LambdaQueryWrapper<ErpPurchaseAlibabaProductitems>();
			query.eq(ErpPurchaseAlibabaProductitems::getProductID, productCode);
			item = iErpPurchaseAlibabaProductitemsService.getOne(query);
			boolean isnew=false;
			if(item==null) {
