...
public class ShipTransCompanyController {
	...
	public Result<String> saveChannelAction(@ApiParam("渠运输方式")@RequestBody List<ShipTransChannel> shipchannels) {
		...
		for(ShipTransChannel shipTransChannel:oldlist) {
			oldMap.put(shipTransChannel.getId(),shipTransChannel);
		}
		for(ShipTransChannel tranChannel:shipchannels) {
			...
			if(tranChannel.idIsNULL()) {
				tranChannel.setId(null);
				QueryWrapper<ShipTransChannel> queryOne=new QueryWrapper<ShipTransChannel>();
				queryOne.eq("shopid", shopid);
				queryOne.eq("name", tranChannel.getName());
				ShipTransChannel old = shipTransChannelService.getOne(queryOne);
				if(old!=null){
					tranChannel.setDisabled(false);
					tranChannel.setId(old.getId());
					oldMap.remove(tranChannel.getId());
				...
				queryOne.eq("name", tranChannel.getName());
				ShipTransChannel old = shipTransChannelService.getOne(queryOne);
				if(old!=null && !old.getId().equals(tranChannel.getId())){
					tranChannel.setDisabled(false);
					tranChannel.setId(old.getId());
					oldMap.remove(tranChannel.getId());
					shipTransChannelService.updateById(tranChannel);
		...
		for( ShipTransChannel oldone:oldMap.values()) {
	...
	@PostMapping(value = "/getTransDetail")
	public Result<Map<String,Object>> getTransDetailAction(@RequestBody Map<String,Object> params){
		return Result.success(shipTransDetailService.searchChannelDetails(params));
	}
}
