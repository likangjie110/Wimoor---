...
public class NotifyController  {
	...
	public Result<Object> submitMsgByUser(@RequestBody MessageDTO dto) {
		Map<String, Object> map = new HashMap<String, Object>();
		UserInfo userInfo = UserInfoContext.get();
		if(dto.getId().equals("0")) {
			dto.setId(iSysUserService.getAdminid());
		}
		int result = notifyHandlerService.createMessage(userInfo, dto);
		if (result > 0) {
			map.put("msg", "创建私信成功!");
		} else {
			map.put("msg", "创建私信失败!");
