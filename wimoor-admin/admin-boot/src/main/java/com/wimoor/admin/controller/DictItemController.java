...
public class DictItemController {
    ...
    public Result<?> list(String name, String dictCode) {
        List<SysDictItem> list = iSysDictItemService.list(
                new LambdaQueryWrapper<SysDictItem>()
                        ...
                        .select(SysDictItem::getName, SysDictItem::getValue,  SysDictItem::getListClass, SysDictItem::getCssClass)
    ...
    public Result<List<SysDictItem>> listType(@PathVariable String typeCode) {
    	  List<SysDictItem> list =null;
            if (!StrUtil.isEmpty(typeCode)) {
             list = iSysDictItemService.list(new LambdaUpdateWrapper<SysDictItem>()
                                              .eq(SysDictItem::getDictCode, typeCode)
                                              .eq(SysDictItem::getStatus, 1));
            }
        return Result.success(list);
    }
    ...
    @GetMapping(value = "/dict/{typeCode}")
    public Result<List<SysDictItem>> listTypeItems(@PathVariable String typeCode) {
        List<SysDictItem> list =null;
        if (!StrUtil.isEmpty(typeCode)) {
