...
import com.wimoor.admin.pojo.dto.MessageDTO;
...
public class NotifyHandlerServiceImpl extends ServiceImpl<SystemNotifyMapper,SystemNotify> implements INotifyHandlerService {
	...
	public Integer createMessage(String receiver, String sender, String shopid, String content, String title) {
		...
		record.setNotify(notify.getId());
		record.setUserid(receiver);
		int result = systemUserNotifyMapper.insert(record);
		SystemUserNotify record2 = new SystemUserNotify();
		record2.setIsread(false);
		record2.setCreatedat(new Date());
		record2.setNotify(notify.getId());
		record2.setUserid(sender);
		systemUserNotifyMapper.insert(record2);
		return result;
	}
	...
	@Override
	public int createMessage(UserInfo userInfo, MessageDTO dto) {
		// TODO Auto-generated method stub
		String content = "" + dto.getName() + ":<span class='text-black'>" + dto.getContent() + "</span>";
		String title = "<span class='text-black' > 账户私信 - 来自账户</span>" + userInfo.getAccount()
					 + "<span class='text-black' > 的私信!</span> ";
		Integer result = createMessage(dto.getId(), userInfo.getId(), userInfo.getCompanyid(), content, title);
		return result;
	}
