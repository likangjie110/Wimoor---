...
public class LoginController {
	...
	String mylogoutAction(HttpServletRequest request,HttpServletResponse response, Model model) {
		...
		try {
			...
			String key =CacheConstants.LOGIN_TOKEN_KEY+jsessionid;
    ...
    private void setSession(String account,String apptype,String sessionkey,String jsonuser){
		...
		stringRedisTemplate.opsForValue().set(CacheConstants.LOGIN_TOKEN_KEY+sessionkey,jsonuser,expiresIn,java.util.concurrent.TimeUnit.SECONDS);
		stringRedisTemplate.opsForValue().set(accountkey,CacheConstants.LOGIN_TOKEN_KEY+sessionkey,expiresIn,java.util.concurrent.TimeUnit.SECONDS);
	}
	@ResponseBody
	...
	public Result<?> verifyWechatAppAction(HttpServletRequest request,HttpServletResponse response) {
		...
		if(StrUtil.isNotBlank(oldjsessionid)&& !oldjsessionid.equals(jsessionid)&&!oldjsessionid.equals("null")){
			try {
				String key =CacheConstants.LOGIN_TOKEN_KEY+ oldjsessionid;
			    stringRedisTemplate.delete(key);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
