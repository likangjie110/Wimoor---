...
public class PictureServiceImpl extends ServiceImpl<PictureMapper,Picture> implements  IPictureService {
	...
	public static String purchanseChangeFilePath = "/erp/file/purchasechange/";
	...
	public Picture uploadPicture(String imgsrc, String destinationPath, String pictureid) throws IOException {
		...
		if (destinationPath.indexOf((storageService.getBucketName()+"/")) == 0) {
			ftppath = destinationPath.substring((storageService.getBucketName()+"/").length(), destinationPath.length());
		...
		if (imgsrc.contains("data:image/") && imgsrc.contains("base64,")) {
			...
			try {
				...
				storageService.putObject(storageService.getBucketName(), ftppath+"/"+imageName, new ByteArrayInputStream(bs));
			}
			try {
				storageService.putObject(storageService.getBucketName(), ftppath+"/"+imageName, new FileInputStream(filePath2));
				if (picture != null) {
					picture.setLocation(destinationPath + imageName);
					picture.setOpttime(new Date());
			...
			if (oldimageName != null && !oldimageName.equals(imageName)) {
				...
				if (list == null || list.size() == 0) {
					try {
						storageService.removeObject(storageService.getBucketName(), ftppath+oldimageName);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	public Picture uploadPicture(InputStream dataInputStream, String sourcePath, String destinationPath, String imageName, String pictureid) throws IOException {
		...
		if (oldimageName != null && !oldimageName.equals(imageName) && !oldimageName.contains("no-img-sm")) {
				try {
					if(destinationPath.indexOf((storageService.getBucketName()+"/")) == 0) {
						storageService.removeObject(storageService.getBucketName(), path+"/"+oldimageName);
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	public Picture downloadPicturelocal(String pictureid, String sourcePath, String destinationPath) throws Exception {
		...
		try {
			storageService.putObject(storageService.getBucketName(),destinationPath+ "/"+imageName, new URL(sourcePath).openStream());
		} catch (Exception e) {
			return null;
		}
		if (GeneralUtil.isEmpty(pictureid)) {
    ...
    public String buildDestinationPath(String destinationPath) {
    	 if (destinationPath.indexOf((storageService.getBucketName()+"/")) == 0) {
				destinationPath = destinationPath.substring((storageService.getBucketName()+"/").length(), destinationPath.length());
				if (destinationPath.charAt(destinationPath.length() - 1) == '/') {
	...
	public Picture downloadPictureMaterial(String pictureid, String sourcePath, String destinationPath) throws Exception {
		...
		try {
			if(StrUtil.isNotEmpty(oldname)) {
				storageService.removeObject(storageService.getBucketName(), destinationPath+"/"+oldname);
			}
			storageService.putObject(storageService.getBucketName(),destinationPath+"/"+imageName, new URL("file:\\"+sourcePath).openStream());
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
