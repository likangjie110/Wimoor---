...
public class SecurityGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Boolean checkAuthority(ServerWebExchange exchange,UserInfo user) {
        ...
        for(String roleCode:list) {
              hasAuthorized =hasAuthorized|| CollectionUtil.isNotEmpty(authorizedRoles) && authorizedRoles.contains(roleCode);
        }
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
          ...
          boolean flag =false;
            ...
            if(token!=null) {
            	flag=stringRedisTemplate.hasKey(CacheConstants.LOGIN_TOKEN_KEY+token);
            }
            if (!flag) {
              	 ServerHttpResponse response = exchange.getResponse();
                return ResponseUtils.writeErrorInfo(response, ResultCode.TOKEN_ACCESS_FORBIDDEN);
            }
            String user = stringRedisTemplate.opsForValue().get(CacheConstants.LOGIN_TOKEN_KEY+token);
            stringRedisTemplate.expire(CacheConstants.LOGIN_TOKEN_KEY+token,3,java.util.concurrent.TimeUnit.HOURS);
            ...
            if(stu.getSession().equals(token)) {
            	if(checkAuthority(exchange,stu)) {
                    ...
                    addHeader( mutate, HEADER_USER_INFO, user);
