...
public interface IQuoteOrderService  extends IService<QuoteOrder> {
    ...
    Boolean deleteOrderShipment(String orderid, String shipmentid);

    Boolean addOrderShipment(List<QuoteOrderShipment> list);

    void downloadOrderExport(SXSSFWorkbook workbook,String orderid);

    String md5(String password);

    List<Map<String,Object>> selectQuoteInfo(String orderid,String supplierid);

    Boolean updateOrder(QuoteOrder dto);
}
