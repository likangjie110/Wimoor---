...
//		List<AmzAdvAuth> advauthlist = amzAdvAuthService.selectByExample(example);
		...
		if(runnables.size()>0){
			AdvUtils.executThreadForAdv(runnables, "requestSnapshot"+exportType+campaignType,5,30);
		}
	}
	...
	public void readSnapshotScheduled() {
			try {
				List<AmzAdvSnapshot> list1 = amzAdvSnapshotMapper.selectAvailableByAdTypeNA( null,
						CampaignType.all);
				AdvUtils.executThreadForAdv( getReadSnapShotRunnable(list1),
						"readSnapshotNA" + AdvResponseRecordType.campaign + CampaignType.all,2,30);
			}catch(Exception e) {
				e.printStackTrace();
			}
	}
	...
	public void readSnapshot() {
		try {
			List<AmzAdvSnapshot> list1 = amzAdvSnapshotMapper.selectAvailableByAdType( null,
					CampaignType.all);
			AdvUtils.executThreadForAdv( getReadSnapShotRunnable(list1),
					"readSnapshot" + AdvResponseRecordType.campaign + CampaignType.all,5,30);
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void readSnapshot(Map<String, Object> map) { 
		...
		if(amzsnap.getStatus()==null||!"COMPLETED".equals(amzsnap.getStatus())) {
			...
			try {
			   ...
			   if(e.getMessage().equals("Exports is expired")||e.getMessage().contains("Snapshot requested is older than 24 hours")) {
	...
	private boolean treatAmzAdvSnapshot(AmzAdvSnapshot record, AmzAdvProfile advProfile) {
		...
		try {
			if(record.getLocation().contains("snapshots-prod-us-east-1.s3.us-east-1.amazonaws.com")){
				result=HttpClientUtil.downloadGZFileUrl(advProfile, record,this);
			}else{
	...
	public void requestStoreBrand() {
		   List<Runnable> runnables = new ArrayList<Runnable>();
	    	...
	    	for (AmzAdvAuth advauth : advauthlist) {
				...
				try {
					this.amzAdvAuthService.captureProfiles(advauth.getGroupid(), advauth.getRegion());
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//			}
	...
	public void requestApiSnapshot() {
		...
		if(runnables.size()>0){
			AdvUtils.executThreadForAdv( runnables, "requestHsaVideoSnapshot",5,30);
		}
	}
	...
	public void requestApiCampBudgetRuleSnapshot() {
		...
		if(runnables.size()>0){
			AdvUtils.executThreadForAdv( runnables, "requestHsaVideoSnapshot",5,300);
		}
	}
	...
	public boolean readSnapshotJSON(AmzAdvProfile profile, AmzAdvSnapshot record, JSONReader jsonReader) {
		try {
				...
				if( AdvExportType.campaigns.equals(record.getRecordtype())) {
					num=amzAdvExportsCampaignsHandlerService.treatJSON(record, jsonReader);
				}else if (AdvExportType.ads.equals(record.getRecordtype())) {
					num=amzAdvExportsAdsHandlerService.treatJSON(record, jsonReader);
				}else if (AdvExportType.adGroups.equals(record.getRecordtype())) {
					num=amzAdvExportsAdGroupsHandlerService.treatJSON(record, jsonReader);
				}else{
					num=amzAdvExportsTargetsHandlerService.treatJSON(record, jsonReader);
				}
