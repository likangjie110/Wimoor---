...
public class ApiBuildServiceImpl implements ApiBuildService {
    ...
    @Autowired
    private AmzAdvProfileMapper amzAdvProfileMapper;
	...
	private void handlerBaseException(BaseException e) {
		...
		if(e!=null && e.getMessage()!=null) {
			if (e.getMessage().contains("Server is busy. Try again later") 
					...
					|| (e.getCode()!=null&&e.getCode().contains("Too Many Requests"))) {
	...
	public String adzAdvInvoicesGet(AmzAdvAuth amzadvauth,AmzAdvProfile profile,String jsonparam) {
		...
		AmzRegion regionObject = amzAdvAuthService.getRegion(amzadvauth.getRegion());
		String url = "https://advertising-api.amazon.com/invoices";
		String contentType = "application/json";
		Map<String, String> header = new HashMap<String, String>();
		header.put("Amazon-Advertising-API-ClientId", regionObject.getClientId());
		header.put("Authorization", amzadvauth.getAccessToken());
	...
	public boolean amzAdvDownloadGZIP(AmzAdvProfile profile, AmzAdvSnapshot record, IAmzAdvSnapshotHandlerService handler) {
			return HttpClientUtil.getGZFileUrl(profile,record,handler);
	}
