...
public class AmzAdvReportHandlerServiceImpl implements IAmzAdvReportHandlerService {
	...
	@Override
	public void requestReportByDate(String authid, String start,String end) {
		List<Runnable> runnables = new ArrayList<Runnable>();
		List<AmzAdvAuth> advauthlist = Arrays.asList(amzAdvAuthService.selectByKey(authid));
		if(advauthlist==null || advauthlist.size()==0){
			return;
		}
		// TODO Auto-generated method stub
					...
					public void run() {
						...
						for (AmzAdvProfile profile : profiles) {
							if(checkReport(profile,type.getSegment())) {
								try {
									if(!type.getDisabled()){
										reportTreat.requestReportParamBuildV3(advauth, profile, type, startdate ,enddate, true);
									}
								}catch(Exception e) {
									e.printStackTrace();
								}
	...
	@Override
	public List<Map<String, Object>> listReport(String authid) {
		return this.amzAdvRequestMapper.listReport(authid);
	}
}
