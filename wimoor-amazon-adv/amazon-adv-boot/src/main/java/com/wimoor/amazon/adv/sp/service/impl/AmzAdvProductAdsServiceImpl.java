...
public class AmzAdvProductAdsServiceImpl extends BaseService<AmzAdvProductads> implements IAmzAdvProductAdsService{
	...
	public List<AmzAdvProductads> amzCreateProductAds(UserInfo user,BigInteger profileId, List<AmzAdvProductads> productAdsList) {
		...
		for (int j = 0; j < productAdsList.size(); j++) {
			AmzAdvProductads item = productAdsList.get(j);
			String sku = item.getSku();
			BigInteger adgroupid = item.getAdgroupid();
			Example example=new Example(AmzAdvProductads.class);
			Criteria crit = example.createCriteria();
			crit.andEqualTo("sku",sku);
			crit.andEqualTo("adgroupid",adgroupid);
			List<AmzAdvProductads> productlist = amzAdvProductadsMapper.selectByExample(example);
			if(productlist!=null && productlist.size()>0){
				throw new BaseException("商品广告创建失败！SKU:"+sku+"已存在广告组中！请勿重复添加。");
			}
		}
	...
	@Override
	public PageList<AmzAdvReportPurchaseProduct> getPurchaseProductAdList(Map<String, Object> map, PageBounds pageBounds) {
		return amzAdvProductadsMapper.getPurchaseProductAdList(map, pageBounds);
	}

	@Override
	public PageList<AmzAdvReportSDPurchaseProduct> getSDPurchaseProductAdList(Map<String, Object> map, PageBounds pageBounds) {
		return amzAdvProductadsMapper.getSDPurchaseProductAdList(map, pageBounds);
	}
