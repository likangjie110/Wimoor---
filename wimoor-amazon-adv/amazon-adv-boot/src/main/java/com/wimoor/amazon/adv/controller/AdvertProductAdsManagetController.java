...
public class AdvertProductAdsManagetController {
	...
	@PostMapping("/getSDPurchaseProductAdList")
	public Result<PageList<AmzAdvReportSDPurchaseProduct>> getSDPurchaseProductAdListAction(@RequestBody QueryForList query){
		UserInfo user = UserInfoContext.get();
		Map<String,Object> map = AdvertController.amzAdvParameterMap(query);
		map.put("shopid", user.getCompanyid());
		PageList<AmzAdvReportSDPurchaseProduct> list = amzAdvProductAdsService.getSDPurchaseProductAdList(map,query.getPageBounds());
		return Result.success(list);
	}

	@PostMapping("/getPurchaseProductAdList")
	public Result<PageList<AmzAdvReportPurchaseProduct>> getPurchaseProductAdListAction(@RequestBody QueryForList query){
		UserInfo user = UserInfoContext.get();
		Map<String,Object> map = AdvertController.amzAdvParameterMap(query);
		map.put("shopid", user.getCompanyid());
		if(StringUtil.isNotEmpty(query.getKeywordid())){
			map.put("keywordid", query.getKeywordid());
		}
		PageList<AmzAdvReportPurchaseProduct> list = amzAdvProductAdsService.getPurchaseProductAdList(map,query.getPageBounds());
		return Result.success(list);
	}
