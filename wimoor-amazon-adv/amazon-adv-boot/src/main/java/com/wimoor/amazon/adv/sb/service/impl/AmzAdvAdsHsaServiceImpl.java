...
public class AmzAdvAdsHsaServiceImpl  extends BaseService<AmzAdvAdsHsa> implements IAmzAdvAdsHsaService {
	...
	@Resource
	AmzAdvAdsAsinHsaMapper amzAdvAdsAsinHsaMapper;
		...
		// TODO Auto-generated method stub
		   ...
		   if (StringUtil.isNotEmpty(response)) {
				...
				for(int i=0;i<success.size();i++) {
					...
					//同步保存到关系表中 解析creative
					handleAdvAdsAisn(adsobj.getAdid(),adsobj.getCreative());
					result.add(adsobj);
				}
	...
	public void handleAdvAdsAisn(BigInteger adid,String creative) {
		if(StrUtil.isNotEmpty(creative)) {
			JSONObject json = GeneralUtil.getJsonObject(creative);
			JSONArray array = json.getJSONArray("asins");
			if(array!=null && array.size()>0) {
				for (int i = 0; i < array.size(); i++) {
					String asin = array.getString(i);
					Example example=new Example(AmzAdvAdsAsinHsa.class);
					Criteria crit = example.createCriteria();
					crit.andEqualTo("asin",asin);
					crit.andEqualTo("adid",adid);
					AmzAdvAdsAsinHsa rec = amzAdvAdsAsinHsaMapper.selectOneByExample(example);
					if(rec==null) {
						AmzAdvAdsAsinHsa record=new AmzAdvAdsAsinHsa();
						record.setAsin(asin);
						record.setAdid(adid);
						amzAdvAdsAsinHsaMapper.insert(record);
					}
	...
	@Override
	public PageList<AmzAdvReportHsaPurchaseProduct> getPurchaseAdsList(Map<String, Object> map, PageBounds pageBounds) {
		return amzAdvAdsHsaMapper.getPurchaseAdsList(map, pageBounds);
	}
