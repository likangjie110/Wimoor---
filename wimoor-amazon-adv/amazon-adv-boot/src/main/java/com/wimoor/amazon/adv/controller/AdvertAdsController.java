...
public class AdvertAdsController {
	...
	@PostMapping("/getPurchaseAdsList")
	public Result<PageList<AmzAdvReportHsaPurchaseProduct>> getPurchaseAdsAction(@RequestBody QueryForList query){
		UserInfo user = UserInfoContext.get();
		Map<String,Object> map = AdvertController.amzAdvParameterMap(query);
		map.put("shopid", user.getCompanyid());
		PageList<AmzAdvReportHsaPurchaseProduct> list = amzAdvAdsHsaService.getPurchaseAdsList(map, query.getPageBounds());
