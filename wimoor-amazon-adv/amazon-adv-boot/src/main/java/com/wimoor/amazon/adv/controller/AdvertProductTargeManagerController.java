	...
	public Result<PageList<Map<String,Object>>> getProductTargeListAction(@ApiParam("查询广告") @RequestBody QueryForList query){
		...
		if(list != null && list.size() > 0) {
			for(Map<String, Object> item : list) {
	           ...
	           if(expression!=null&&(expression.toString().contains("asinCategorySameAs")||expression.toString().contains("ASIN_CATEGORY_SAME_AS") )){
	       		    ...
	       		    if(campaignType!=null&&"sd".equals(campaignType.toString().toLowerCase())) {
	       		     if(jsonarr.size()>0) {
		       		     ...
		       		     if(category!=null) {
			        		   item.put("expressionname","ASIN_CATEGORY_SAME_AS="+category.getName());
			        	   }
		       		    }
	       		    }else {
	       			    if(jsonarr.size()>0) {
